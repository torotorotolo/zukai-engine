# -*- coding: utf-8 -*-
"""15本目 リノ・エアレース2011 ⑤a-1：台本第2版 §4 を tools/narration.py の SCRIPT へ**機械で**写す（2026-09-26）。

  python ref/ep15/script_copy.py          … 写して、写した結果を import し直して突き合わせる
  python ref/ep15/script_copy.py --check  … 写さずに突き合わせだけ（食い違いがあれば exit 1）

14本目の `ref/ep14/script_copy.py` を写した（13本目の `qa_out/ep13_script_copy.py` が元）。変えたのは2点:
  ① 台本の md ＝ `ref/ep15/daihon_v2.md`
  ② 期待値（192カット／425行／11,286字／決め所16・聞き役56＝⑤a-1 の頭で final15.py と check_script を自分で回した値。字は印を除く）
以下は14本目の注のまま:
  ③ 🔴 **聞き役の印 `Q: ` を付けたまま写す**（話者の正本は SCRIPT。外すのは使う側が tools/speaker.py を通して）。
     `check_script.clean()` は 14本目⑤aから印も外すので、そのまま写すと**話者が消える**＝印だけ付け直す
共通エンディング（ed01）を残す仕組みは13本目のまま（いまの SCRIPT から字面のまま切り出して末尾へ戻す）。
...
MD = ROOT / "ref" / "ep15" / "daihon_v2.md"
NARR = ROOT / "tools" / "narration.py"
WANT = {"cuts": 192, "lines": 425, "chars": 11286, "quotes": 16, "q_lines": 56}
...
def from_md():
    ...
    # 🔴 聞き役の行だけ印を付け直す（clean は★・**・印を外す）
    out = [(cid, pic, [(speaker.Q_MARK if speaker.is_q(l) else "") + CSC.clean(l) for l in ls])
...
def compare(cuts, nq, tail_want):
    ...
    n_chars = sum(len(speaker.bare(t)) for _, ls in got for t in ls)       # 印は数えない
    n_q = sum(1 for _, ls in got for t in ls if speaker.is_q(t))
    print(f"SCRIPT: {len(got)}カット／{n_lines}行（うち聞き役 {n_q}）／{n_chars}字（印を除く・md の決め所 {nq}）"
          f"＋共通エンディング {len(tail_got)}カット")
    real = {"cuts": len(got), "lines": n_lines, "chars": n_chars, "quotes": nq, "q_lines": n_q}
    if real != WANT:
        bad.append(f"数が第2版の実測と違う: {real} ／ 期待 {WANT}")
    if bad:
        print("🔴 食い違い:\n  " + "\n  ".join(bad[:20]))
        return 1
    print("✓ md と SCRIPT は1文字も違わない（192／425／11,286／16・聞き役56）・ed01 は元のまま")
    return 0
