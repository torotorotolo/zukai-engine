...
| ファイル | 状態 | 判定 |
|---|---|---|
| `titan_hull_pair.jpg` (1609×1490) | サムネの「2枚並べ案」（コミット `655f550`）で取得したが、**出所を記録しないままコミットされていた** | ❌ **使わない**。本編226カットのどこにも割り当てていない |

⚠️ すでに public リポジトリの履歴に入っているので、履歴からは消していない。
**画面に出さないことで対処する**（出典を出せない写真は使わない、というこの台帳の原則どおり）。
