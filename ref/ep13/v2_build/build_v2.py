# -*- coding: utf-8 -*-
"""13本目④'：台本 第1版（読むだけ）に差し替え表を当てて第2版を組む。
  - 第1版の「## 1. 🔴 ④で決めたこと」から後ろを本体にし、頭と §0 は v2_head_*.md に差し替える
  - v2_sect*.txt（@@@OLD/@@@NEW/@@@END）＝節の差し替え。OLD は本体にちょうど1回出ること（違えば止まる）
  - v2_cuts_*.txt（@@ cNNN ／ @@ +cNNN after cMMM）＝カットの差し替え・追加。元のカットがちょうど1つ在ること
終了コード 0=組めた / 2=当て損ない
"""
...
R = Path("C:/Users/konar/Desktop/zukai-engine/ref/ep13")
...
n_rep = n_add = 0
ids_seen = []
for f in sorted(S.glob("v2_cuts_*.txt")):
    t = f.read_text(encoding="utf-8")
    for blk in re.split(r"^@@ ", t, flags=re.M)[1:]:
        head, rest = blk.split("\n", 1)
        new = rest.strip("\n") + "\n"
        m = re.match(r"\+(c\d{3}) after (c\d{3})", head.strip())
        if m:
            nid, after = m.groups()
...
head = "".join((S / n).read_text(encoding="utf-8") for n in ("v2_head_a.md", "v2_head_b.md"))
...
print(f"節の差し替え {n_sect} ／ カットの差し替え {n_rep} ／ 追加 {n_add} ／ 🔧 の見出し {len(re.findall(r'^\*\*c\d{3}\*\* 🔧', out, re.M))}")
print("第1版 md5", md5_before)
if err:
    print("\n".join("E " + e for e in err))
    sys.exit(2)
(R / "daihon_v2.md").write_text(out, encoding="utf-8")
print("→", R / "daihon_v2.md", len(out), "字")
