# -*- coding: utf-8 -*-
"""14本目 セウォル号 ⑤a：台本第2版 §4 を tools/narration.py の SCRIPT へ**機械で**写す（2026-09-25）。

  python ref/ep14/script_copy.py          … 写して、写した結果を import し直して突き合わせる
  python ref/ep14/script_copy.py --check  … 写さずに突き合わせだけ（食い違いがあれば exit 1）

13本目の `qa_out/ep13_script_copy.py` を写した。変えたのは3点:
  ① 台本の md ＝ `ref/ep14/daihon_v2.md`
  ② 期待値（195カット／439行／11,166字／決め所16＝⑤aの頭で check_script を自分で回した値。字は印を除く）
...
MD = ROOT / "ref" / "ep14" / "daihon_v2.md"
NARR = ROOT / "tools" / "narration.py"
WANT = {"cuts": 195, "lines": 439, "chars": 11166, "quotes": 16, "q_lines": 56}
...
def compare(cuts, nq, tail_want):
    ...
    if bad:
        print("🔴 食い違い:\n  " + "\n  ".join(bad[:20]))
        return 1
    print("✓ md と SCRIPT は1文字も違わない（195／439／11,166／16・聞き役56）・ed01 は元のまま")
    return 0
