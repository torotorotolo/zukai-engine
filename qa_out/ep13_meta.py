...
# 🔴 2026-09-25 カズヤくん決定＝サムネ b（地は姉妹機 TC-JAU＝事故機そのものではない）。
#    CC BY 3.0 の写真に文字を重ねた＝改変の表示が要る。事故機の写真と誤解されないよう、あわせて断る
#    （10本目の「※ サムネイルの画像はイメージです」と同じ考え方）。
TAIL = """動画に出てくる数値・時刻・証言の言葉は、これらの記録によります。
資料によって数が違うところ、報告書が言い切っていないところは、画面でそのまま断っています。
※ サムネイルは、事故機の姉妹機 TC-JAU（同じトルコ航空の DC-10・1974年／RuthAS・CC BY 3.0）の写真に、文字を重ねたものです。事故機そのものの写真ではありません。"""
...
def main() -> int:
    ...
    # ✅ 2026-09-25 カズヤくん決定＝**b**（赤「犠牲346人 旅客機の半ドア」＋姉妹機 TC-JAU）。4案から1巡目で決まった
    #    （Actions run 36137239684・`580a616`）。原寸で字の欠け・顔なしを確かめた
    thumb = "out/thumb/ep13-t1/ep13_b_handoa_thy.png"
    meta = {
        "slug": "ep13",
        "title": TITLE,
        "description": desc,
