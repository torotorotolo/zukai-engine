# -*- coding: utf-8 -*-
"""9本目（テネリフェ）⑤b の作業表。**台本第2版と narration.json を突き合わせて出す。**

■ なぜ要るか
  215カットを台本の本文で読み直すと 1,600行を毎回抱えることになる。
  ⑤b が要るのは「カットID／画の欄／出典／行数／秒」の5つだけなので、それだけを出す。
  （8本目の `qa_out/ep8_plan.py` を台本の置き場だけ替えて写した）

■ 検算（fail closed）
  台本のカットID列と `audio/narration.json` の `durations` のキー列が
  **順番も込みで一致**しなければ止まる。片方だけ直したまま⑤bを進める事故を防ぐ。

    python qa_out/ep9_plan.py            # 章ごとの表
    python qa_out/ep9_plan.py --photo    # 「実写」の欄だけ
    python qa_out/ep9_plan.py --fig      # 図の欄だけ（型ごとに数える）
    python qa_out/ep9_plan.py --lines    # 字幕の行も出す（図の中身を決めるとき）
"""
...
SCRIPT_MD = Path(
    ...
    r"\事故検証-テネリフェ-台本第2版-20260916.md")
