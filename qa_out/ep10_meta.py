...
# 🔴 2026-09-22 カズヤくん指示で末尾に1行足した。
#    サムネイルの地を、崩落の瞬間を描いた**生成画像**に差し替えたため
#    （崩落の瞬間を撮った写真・映像は1枚も残っていない）。
#    このチャンネルは「公の記録だけを使う」ことを売りにしているので、
#    **本物の記録写真と誤解されないように、概要欄で断る。**
#    ⚠️ 生成画像を使うのは**サムネイルだけ**。本編の画には1カットも入れていない。
TAIL = """動画に出てくる数値・時刻・判決の言葉は、これらの記録によります。
資料によって数が違うところ（崩壊の時刻、冷却塔の数、亡くなった人の数）は、画面でそのまま断っています。
※ サムネイルの画像はイメージです。崩落の瞬間を記録した写真・映像は残っていません。本編で使っている写真は、すべて実際の記録です。"""
...
def main() -> int:
    ...
    # 🔴 2026-09-22 カズヤくん決定＝**A/B テストを終了し、崩落の瞬間の絵に差し替える**。
    #    地＝`tools/gen_thumb_ai.py e`（生成画像）→ `thumb_jiko.py ep10-ai2` で赤・黄を焼く。
    #    ⚠️ 差し替えは**カズヤくんが手で**行う（このファイルは概要欄の本文を出すために回す）。
    #    旧＝`out/thumb/ep10-t2/ep10_g_split.png`（2026-09-21 採用・C: の掃除で実体は消えている）
    thumb = "out/thumb/ep10-ai2/ep10ai_e_kuzure.png"
    meta = {
        "slug": "ep10",
        "title": TITLE,
        "description": desc,
