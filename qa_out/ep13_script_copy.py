# -*- coding: utf-8 -*-
"""13本目 トルコ航空981便 ⑤a：台本第2版 §4 を tools/narration.py の SCRIPT へ**機械で**写す（2026-09-24）。

  python qa_out/ep13_script_copy.py          … 写して、写した結果を import し直して突き合わせる
  python qa_out/ep13_script_copy.py --check  … 写さずに突き合わせだけ（食い違いがあれば exit 1）

12本目の `qa_out/ep12_script_copy.py` を写した。変えたのは2点だけ:
  ① 台本の md ＝**リポの `ref/ep13/daihon_v2.md`**（12本目までは Vault に置いていた。13本目は④からリポ）
  ② 期待値（192カット／386行／11,731字／決め所16＝⑤aの頭で check_script を自分で回した値）
共通エンディング（ed01）を残す仕組みは12本目のまま（いまの SCRIPT から字面のまま切り出して末尾へ戻す）。
...
MD = ROOT / "ref" / "ep13" / "daihon_v2.md"
NARR = ROOT / "tools" / "narration.py"
WANT = {"cuts": 192, "lines": 386, "chars": 11731, "quotes": 16}
...
def compare(cuts, nq, tail_want):
    ...
    if bad:
        print("🔴 食い違い:\n  " + "\n  ".join(bad[:20]))
        return 1
    print("✓ md と SCRIPT は1文字も違わない（192／386／11,731／16）・ed01 は元のまま")
    return 0
