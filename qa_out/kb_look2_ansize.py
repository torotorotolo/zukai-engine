# -*- coding: utf-8 -*-
"""🔴🔴 **使わない。`qa_out/kb_u_meter.py ann` を使うこと。**（2026-09-10 ⑤c' 2巡目に判明）

  この物差しは `photo_ann()` の**式を書き写している**ので、`photo_ann()` を直しても
  **同じ数字を出し続けた**（直した直後も「62組」と出た。実物は 0組）。
  ＝ [[feedback-verify-your-own-instrument]]「式を書き写す物差しは直しても同じ数字を出す」。
  代わりの物差しは `photo_ann()` を**本当に回して** `<text font-size=…>` を読む。
  ⚠️ 記録のために残してあるだけ。**判断に使わない。**
