...
■ 使い方
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # ⓪ 🔴 **必ず先に門番を通す**（2026-08-02 追加。「絶対に課金しない」の担保）
    #    請求が発生しうるとき、予算に当たって途中で切られるときは、押す前に止まる。
    export PYTHONIOENCODING=utf-8 PYTHONUTF8=1     # ← これも必須（下記）
    python tools/guard_cost.py --est 0.30 || echo "回してはいけない"
...
🔴 **Windows から叩くときは `PYTHONIOENCODING=utf-8 PYTHONUTF8=1` を必ず付ける。**
   付けないと Modal CLI が出す `✓` を cp932 が扱えず、**リモートに到達する前に
   クライアントごと落ちる**（2026-08-02 に実際に1回それで失敗した。
   [[reference-modal-blender-render]] に既に書いてあったのに読まずに踏んだ）。
