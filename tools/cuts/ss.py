...
🔴🔴 2026-09-08（6本目②）：**このファイルの素材定数は 5本目（SL-1）のものです。**
   章ファイル（`pr` `c1`〜`c9` `ep`）は空にしたので、いまは誰も import していません。
   ⚠️ **`IDO_P16` などを 6本目でそのまま使わない**（`ref/sl1/` の報告書のページです）。
   ④で 6本目の素材に書き替える。⇒ 逆算の関数（`focus()` など）はそのまま使えます。
