...
NG_PHOTOS: dict[str, str] = {
    ...
    #    ✅ **2026-09-22（⑤c-3）で欄ごと外した。**本物を別の置き場で見つけたため。
    #       Internet Archive の蔵書スキャン `reporttopreside00unit` の表紙に差し替え、
    #       絵を見て確かめた（大統領章＋青い表紙＋「Report of the PRESIDENTIAL
    #       COMMISSION on the Space Shuttle Challenger Accident」）。
    #    🔴 落とした Commons の点は `qa_out/ep11_assets.py` の `PICK` からも消えたので、
    #       **同じ画素が入ってくる経路はもう無い**（欄名の中身ごと別物に置き換わった）。
    #       ⚠️ だから名前で止める網はここでは効かない。効くのは `check_blank`（インク率）と
    #          ⑤c の原寸目視のほう。**この欄の発見はその2つに任せる。**
