...
SPEC = {
    ...
    "c718": dict(
        ...
        fig=("people", dict(
            ...
            # 🔴 2026-09-17（⑤c' E-03）：段が7つ（矢印3→箱4）で、オランダの箱が尺の最後 0.40秒だけ
            #    だった＝節と矢印を1つの段にまとめる（段 4）
            pair=True,
            note="事故報告書 p32")),
    ),
    ...
    "c726": dict(
        ...
        fig=("beforeafter", dict(
            ...
            # ⚠️ 2026-09-17（⑤c B-35）：lead「答えは、報告書の別の頁（p27）」が問い（語りの最後）より
            #    先に答えの在りかを出していた・「頁」は読みにくい＝外す（答えは次の c727）
            arrow=False,
            note="事故報告書 p32")),
    ),
    ...
    "c727": dict(
        t="目に気を取られ、耳がおろそかに",
        s="報告書が別の場所で書いた答え",
        # ⚠️ 2026-09-17（⑤c B-33）：process の矢印が「聞くほうが落ちる（KLM）」→「パンナムに
        #    届いたのは」を原因と結果に見せた（語りは「KLMだけではなかった」＝並べた話）。
        #    出典「事故報告書 p27」が中身の欄に入っていた。→ 矢印の無い、並べる図にする。
        fig=("beforeafter", dict(
            a=dict(k="KLM", t="聞くほうが落ちる", lines=["見ることに気を取られる", "霧の中の滑走路"],
                   c=J.AMBER),
            b=dict(k="パンナム", t="届いたのは、最後の一音", lines=["管制官の送信のうち"],
                   c=J.INK_W),
            arrow=False, note="事故報告書 p27・p38")),
    ),
