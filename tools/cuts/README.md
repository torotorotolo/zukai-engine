...
## 5. 🔴 195カットを1から書いて、門番に教わったこと（2026-09-20・10本目 ⑤b-2）

**書き始める前にここを読むと、同じ手戻りを1周ぶん減らせる。**
下は全部、**机上の門番が実際に鳴らして**分かったもの（推測ではない）。

### 5-1. 見出し・副題は「言い換え」であって「切り取り」ではない

`check_echo` は**見出し・副題・図の札のすべてを、字幕の1文ずつと突き合わせる**。
字幕の文から**そのまま切り取った見出し**は鳴る（実測：pr10 は一致率 **100%**、pr11 は **91%**）。

| 場所 | 持つもの | ⚠️ 落とし穴 |
|---|---|---|
| 見出し（t） | そのカットの**主張**を、**別の言葉で** | 字幕の頭か尻をそのまま取ると鳴る |
| 副題（s） | **何を見ている図か**（時・場所・出どころ・対象） | 図の札と**同じ語**を入れると `check_dup` が鳴る（pr06「模式図」） |
| 図の札 | **数値・場所・関係・出どころ**。文を書かない | 「5階の食堂の床が盛り上がる」は文＝鳴る。「5階　食堂の床」にする |

🔴 **字幕の行が短いほど危ない。**「12倍をこえている。」（9字）のような行は、
数字を見出しに入れるだけで一致率が 67% を超える。**短い行の語は、見出しでも札でも使わない。**

### 5-2. `check_wording` A型 ── 見出し・副題に「段の答え」を書かない

段（`ann` の `v`／`panel` の `v`）が出す答えを、見出しや副題が**先に言ってしまう**と鳴る。
実測 11件。いちばん多かったのは**章の橋**だった：

    ✗ 副題「第4章へ」＋ 段 v「第4章」   → A型
    ✓ 副題「ここまでと、この先」＋ 段 v「第4章」

→ **章の橋の副題は「ここまでと、この先」でそろえてある**（10本目の8カット）。

### 5-3. 型ごとの、書いて初めて分かる決まり

| 型 | ⚠️ |
|---|---|
| `panel` | **段は `t` が必須**（`k` は省ける）。`t` を落とすと `KeyError: 't'` で**全部の検査が落ちる** |
| `compare` | **差が小さいと例外で止まる**（「棒は全部同じ長さになる＝図が『同じだ』と言ってしまう」）。実測 71,136 対 73,205 ＝ 差 2.8% で停止 → **`bar=False`** で数字に語らせる |
| `compare` | `vmax` を渡さないと、その場の最大値が満杯の棒になる（README §2 と同じ） |
| `timeline` | 出来事が `t0` に近すぎると、**札が左下の出典と重なる**（c318）。`t0` を左へ広げて逃がす |
| `punch` | **段が自分で札を書く**（段2「上に載る重さ」／段4「触れているのは、この幅だけ」／段6「破壊面」／段7「外れて落ちる」）。`lead` で同じことを言わない |
| 図の `note` | **長いと2行に折れて、型が自前で置いた札と重なる**（c113）。出典は短い形で書く |

### 5-4. 🔴🔴 画面にハングルを書かない

`check_layout` の「フォントに無い字（豆腐になる）」が鳴る（実測：`96도1231` の `도`）。
韓国の判例番号・法令名は**日本語で**書く（「大法院 1996年8月23日 判決（刑事）」「韓国 法制処」）。

### 5-5. 🔴🔴 **書ける形で並んでいる引数が、描く側で実装されているとは限らない**

`cuts/ss.py` の `_BREAKS_FRAME` に `"blur"` が並んでいるので**章ファイルに書けるように見える**が、
`scene_jiko` も `build_jiko` も `blur` を読まない（`grep -rn blur tools/*.py` で確かめた）。
＝ **`blur=` と書いても、エラーも出さずに素のまま焼ける。**
10本目はこれが**私人の顔写真と名前**に当たっていた（`missing_board_01`／`c810`）。

→ 隠すのは**元画像そのもの**。黙って素のまま焼かないように **`tools/check_photo_mask.py`** を
　 新設し、`qa_all` に載せた（`cuts/ss.py` の `NEEDS_MASK` ＋ `ref/ep10/masked.json` の md5）。
→ [[feedback-settings-may-not-reach-the-picture]]／[[feedback-rules-need-gates]]

⚠️ **新しい引数を書く前に、描く側が読んでいるか `grep` する。**
