...
SPEC = {
    ...
    "c609": dict(
        ...
        fig=("compare", dict(
            ...
            vmax=14,
            # 🔴🔴 2026-09-22 ⑤c''：**`bar=False` にした（c518 と同じ直し）。**
            #    `compare` の棒は `abs()` で立つので、**−3 も基線から上向きの棒**になり、
            #    焼いた絵の実測＝11.7 が 276px・−3 が 74px（比は |値| と合っている）。
            #    ＝絵は「ずっと少ない」としか言っておらず、見出し「**線の、はるか下だった**」を
            #    否定していた（[[feedback-charts-lie-with-their-defaults]]＝符号が逆の値を
            #    同じ向きの棒で並べない）。台帳 §10-3 は c518 を🔴🔴・ここを🔴弱いとして
            #    棒を残したが、⑤c'' で原寸を見ると**同じ型の嘘**だった。
            # ⚠️ 数字は `11.7` と `−3`（マイナス付き・赤）で大きく出ており、
    ...
    "c613": dict(
        ...
        # 🔴 2026-09-22 ⑤c'（10-6）：手前の**看板に `George C. Marshall Space Flight Center`**
        #    が文字高 約45px で読めた。看板は元画像の x 0.06〜0.26 にあるので、左を切り落とす。
        # ⚠️ 切ったあと 2160x2002。覆いは横を落とさず縦を 61% に切るので、bias=0.25 で
        #    y 0.118〜0.725＝**建物（y 0.13〜0.55）がまるごと残る**（`crop_probe` で実測）。
        # ⚠️ bias は 0.25 では**門番 `check_slide` G-14 が鳴った**＝右の木立を OCR が
        #    文字行と読み（元 y 0.259〜0.275）、注記「ここにいた」と 102×17px 重なった。
        #    0.30 にすると その行は画面 y251〜280・次の行は 375 で、注記（300〜330）から
        #    上下とも 20px 以上あく（覆いの式で計算してから採った）。
        trim=(0.28, 0.0, 1.0, 1.0), bias=0.30,
        # ⚠️ 注記を 300 → 500 へ下げた。OCR は**右の木立を文字行と読む**（元画像の
        #    x 0.646〜0.925／y 0.203〜0.355＝画面では y 151〜424 の帯）。切り方を変えたら
        #    その帯が注記の高さに来て `check_slide` G-14 が 102x17px で鳴った。
        #    500 なら帯の下（424）より下に出る。⚠️ 木の葉であって本物の文字ではないが、
        #    **赤いまま置くと⑥で本物が埋もれる**ので避ける（[[feedback-gates-blind-spot-is-the-scan-direction]]）。
        side="right", ann_y=500,
    ...
    "c618": dict(
        t="いまの基準のままで来た",
        s="それまでの打ち上げの回数",
        # 🔴 2026-09-22 ⑤c'（10-9 の14）：点は 8列x3行＝24個で**数は合っていた**が、
        #    ①「24」の数字が画面に0文字 ②24個すべて `oncol` の既定＝**警告の赤**、の2つ。
        #    見出しは「いまの基準のままで来た」＝**成功してきた回数**なので、赤は図が嘘をつく。
        #    （[[feedback-charts-lie-with-their-defaults]]／c402 と同じ既定の罠）
        # ⚠️ 数はナレーションも見出しも言っていないので、`lead` に置いても字幕の写しにならない。
        fig=("icons", dict(
            n=24, on=24, kind="dot", cols=8, oncol=J.OK,
            lead="いまの基準で成功してきた回数　24回",
            note=SRC5)),
    ),
