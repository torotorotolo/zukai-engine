...
SPEC = {
    ...
    "c501": dict(
        t="括弧の外の3つを、順に",
        s="p86 の右側の3つの箱",
        # 2026-09-06（6）🔴 焼いた絵を見て分かった：**主役の3つめの箱（Degradation over time）が
        #   右端で半分切れ**、注記3/4/5 がその上に重なっていた。門番は G-10 2件しか出しておらず、
        #   絵を見るまで軽く見えていた（[[feedback-desk-checks-dont-see-pictures]]）。
        #   ⚠️ スライドの見出しは**どの切り方でも入らない**（原画 3200 幅に対し 16:9 の最大切り出しは
        #   2791px、見出しは 2869px）。→ 見出しは枠の外へ出す＝top>427 になる zoom を採る。
        #   3つの箱（原画 x 1371〜3088）が k=0/k=1 の両方で丸ごと入る値。注記は箱の下（ann_y=560）へ。
        photo=ss.P086, side="right", ann_y=560,
        xbias=0.93, bias=1.0, zoom=1.45,
    ...
    "c502": dict(
        ...
        **ss.focus(ss.P086, 0.50, 0.62, 2.6),
        ...
        ann=[dict(t="重さの出どころ", dy=40, d="土・水・育つ植木が、まるごと載る", dc=J.LINE,
                  ds=28)],
    ),
    ...
    "c507": dict(
        ...
        **ss.focus(ss.P086, 0.70, 0.62, 2.6),
        # ⚠️ G-09：t は箱の英文の丸写し。variable の1語だけ読み方を残す
                # 2026-09-06（6）G-14：注記が焼き込みの英文の上に載っていた。dy で逃がす（枠外にならないことは check_layout の実測箱で確認）
        ann=[dict(t="足された厚み", dy=120, d="variable＝場所ごとに違う", dc=J.LINE, ds=30)],
    ),
    ...
    "c518": dict(
        ...
        **ss.focus(ss.P174, 0.25, 0.50, 1.8),
        # ⚠️ E-09/G-09：t が画面の "hundreds" の真上に載って同じ語が二重だった
                # 2026-09-06（6）G-14：注記が焼き込みの英文の上に載っていた。dy で逃がす（枠外にならないことは check_layout の実測箱で確認）
        ann=[dict(t="標本の数", dy=80, d="数百本の単位で調べた", dc=J.INK_W, ds=30)],
    ),
