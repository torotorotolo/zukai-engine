...
SPEC = {
    ...
    "c517": dict(
        ...
        # ⑤c'（09-23）：下の辺が図の注「Figure 65. …24 hours after」を横に切っていた（原寸 p20）＝注は副題が名乗るので外す（台帳 §7-4）
        photo=ss.page(230), panel=True, trim=(0.01, 0.05, 0.99, 0.805),
        side="right",
    ...
    # ⑤c'（09-23）：上の端のフィルムの黒い縁に手書きのネガ番号「22-1287 UNCL」（縁は高さの約4.8%まで）
    #    ＝資料の注記（ルール §5b-5）＝縁ごと切る（台帳 §7-6 D）。OCR は手書きを読まない（§5c-22）
    "c518": dict(
        t="指令所から、救出",
        s="艦カーチスで服を脱いだあとの測定　1954年",
        photo=P("dc_curtis"), **ss.kind(P("dc_curtis")), trim=(0.0, 0.056, 1.0, 1.0),
        side="right",
    ...
    # ⚠️ ⑤b-4：カメラが東へ寄り切ると題「Figure 61.」（y 0.850〜0.872・x 0.133〜）の頭が左端で35px欠けた（check_slide G-10）
    #    ＝題の手前（y 0.845）で切る。図の番号と題は副題が名乗っている
    "c520": dict(
        t="灰の先頭は、島を越えた",
        s="報告書の第61図　6時間後の降灰の再現",
        # ⑤c'（09-23）：窓の下の辺が縮尺の数字「0 20 40…」を横に切ったまま流れていた（見えて50〜63%）＝窓を下げて数字を丸ごと入れる（台帳 §7-4）
        photo=ss.page(223), panel=True, trim=(0.02, 0.55, 0.99, 0.85),
        cam=EASTWARD,
        side="right",
