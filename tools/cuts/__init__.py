...
■ 🔴🔴 2026-09-23（12本目 キャッスル・ブラボー ⑤b-2）：**12本目へ差し替え。**
  11本目の章ファイル・ss.py・BACKDROP は git の `61039d2` にある。
  12本目は **pr と ep のカットが無い**（第1章 c101〜第9章 c920 の190カット）＋共通の末尾 `ed01`。
  検算：空にした直後は `len(SPEC)` が **1**（ed01 だけ）。書き終えたら **191**。

■ 🔴🔴 2026-09-24（13本目 トルコ航空981便 ⑤b-1）：**13本目へ差し替え。**
...
BACKDROP = {
    # 🔴🔴 2026-09-23（12本目 キャッスル・ブラボー ⑤b-2）**11本目の3件を空にした。**
    #    中身は git `61039d2`（`git show 61039d2:tools/cuts/__init__.py`）。
    #    ⚠️ 11本目の c513・c517・c606 は**12本目にも同じIDがある**＝残すとチャレンジャー号の
    #       継ぎ目の写真が、ブラボーの図の地に黙って敷かれる。
    #    12本目の割り当ては⑤b-2 で章ファイルを書いてから決める（その話の対象そのものだけ）。
}
