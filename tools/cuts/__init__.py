...
BACKDROP = {
    ...
    #   ▼ 🔴🔴 **1回目の直し（y 0.20〜0.51 の帯）は効いていなかった。**焼いて原寸で見たら
    #     記章のまわりを**回り込むように書かれた乗員名 "CHAWLA" "ANDERSON" が残っていて、
    #     本文がその上を通っていた**。`ep8_burnedin2.py` は「c913 は0行」と言っていた。
    #     ＝ **OCR は回転した文字を行として拾わない**ので、OCR の箱から切る所を決めると
    #       見落とす（[[feedback-absence-of-a-word-is-not-absence]]）。**絵が正本。**
    #   → 原寸から座標を逆算した：CHAWLA は素材の x≈0.58・ANDERSON は x≈0.71。
    #     記章そのものも x 0.55〜0.80。**x を 0.50 で切れば記章ごと外へ出る**
    #     （HUSBAND の 0.531 まで 48px・CHAWLA まで 128px の余白）。
    #     縦は題字（〜0.179）と下端の "REPORT VOLUME I"（0.78〜）のあいだ。
    #   ⚠️ 倍率 2.605（拡大）。表紙の左半分＝星だけの無地なので、暗幕 0.86 の地紋には
    #     むしろ向いている（絵柄が騒がしくない）。
    "c913": dict(photo=ss.REPORT_COPY, veil=0.86, bias=0.50, zoom=1.00,
                 trim=(0.02, 0.21, 0.50, 0.67)),
}
