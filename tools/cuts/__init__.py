...
BACKDROP = {
    # 🔴 2026-09-22（11本目 ⑤c-3）**3件だけ**。9本目（テネリフェ）の21件は git `e18b8f1`。
    #    3件とも「数値や関係は図が語り、**その話の対象そのもの**を地に沈める」形。
    #    ⚠️ 全カットには敷かない（敷いた瞬間に図解チャンネルである意味が消える）。
    #    ⚠️ 3件とも記録映画の継ぎ目の寄りだが、**別のショットの別のコマ**
    #       （c513＝元1560・c517＝元1596・c606＝元1554）。同じ画にならないことは `check_twin` で見る。

    # c513「開く量は、輪ごとに違った」＝開くのは、この継ぎ目のすきま
    "c513": dict(photo=ss.P("joint_test"), veil=0.86, bias=0.50),

    # c517「開く速さと、戻る速さ」＝ふさぐ相手は、この継ぎ目のすきま
    "c517": dict(photo=ss.P("oring_resilience_test"), veil=0.86, bias=0.50),

    # c606「跡の広さが、まるで違った」＝黒い跡が残ったのは、この継ぎ目のゴムのあいだ
    #    ⚠️ 地は**51-C の現物ではない**ので、跡そのものは図だけが語る（副題も名乗らない）。
    "c606": dict(photo=ss.P("oring_soot"), veil=0.86, bias=0.50),
}
