...
SPEC = {
    ...
    # ── c804 濃く落ちた帯 ─────────────────────────────────
    # 🔴🔴 2026-09-15（⑤c''）：**黄の破線が札の字の上を通っていた。**
    #    ⑤c' の実測＝「フォートワースの南」は **555画素／札の幅の 49%**、
    #    「ルイジアナ」は **177画素／24%**（2件目は⑤c が挙げていない）。取り消し線に見える。
    #    ⚠️ `check_layout` の「図形が文字を横切る」は、**文字があとの層に描かれると
    #      『読めるから良い』として飛ばす**ので1件も鳴らない（既知の穴）。
    #    → `mapfig` は 2026-09-10（6本目 c105）で **札の side を選べるようにしてある**。
    #      破線は点1（x=0.42）から右下の点2（x=0.86）へ伸びるので、
    #      **点1の札を左へ／点2の札を右へ**逃がす（＝線の反対側）。
    #      点の位置は動かさない（地図の南北・東西の約束に触らない）。
    #    ⚠️ 逃がした先で隣とぶつからないか計算した：点1の左札は x 484〜790、
    #      点0の補足は x 349〜687 だが、y は 472 対 514 で 42px 離れる（重ならない）。
    #      点2の右札は x 1605〜1787 で BX1=1848 の内側。
    "c804": dict(
        t="濃い帯は、南から東へ伸びた",
        s="破片が濃く落ちた帯",
        fig=("mapfig", dict(
            points=[dict(x=0.12, y=0.30, t="リトルフィールド",
                    ...
                    dict(x=0.42, y=0.44, t="フォートワースの南",
                         d="濃い帯の始まり", c=J.ALERT, side="left"),
                    dict(x=0.86, y=0.62, t="ルイジアナ", d="濃い帯の終わり",
                         c=J.ALERT, side="right")],
            link=(1, 2),
    ...
    # 🔴🔴 2026-09-15（⑤c''）：**c707 と同じ絵だった**（素材 `oex_recorder`・窓が同一。
    #    ⑤c' が画素で確かめ、**貼り箱だけ左右反対**（x=72 対 x=1200）と分かった
    #    ＝置き場所が違うだけで絵は1画素も違わない）
    #    → [[feedback-duplicate-art-needs-pixel-comparison]]。
    #    → **c707 は見終わっている版なので触らず、こちら（c817）だけ窓を変える。**
    #      素材を測ったら、装置（暗い箱）は x 0.12〜0.83・y 0.38〜0.72 にあり、
    #      上端（y<0.30）と下端（y>0.96）は**明るい帯**で装置ではない。
    #      → 0.08〜0.92 × 0.16〜0.84 に切って、その帯を落とす。
    #      箱は 648×648 → **801×648**（横長）になるので、並べても別の絵と分かる。
    #    ⚠️ 倍率 0.621（縮小）なのでぼやけない。装置の外側に 0.04 ぶんの余白を残した。
    "c817": dict(
        t="3月19日、記録装置が出てきた",
        s="同型の記録装置　1988年撮影",
        photo=ss.OEX_RECORDER, side="left", ann_y=300,
        trim=(0.08, 0.16, 0.92, 0.84),
