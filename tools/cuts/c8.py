...
SPEC = {
    ...
    # 🔴🔴 2026-09-22 ⑤c'（10-5）：**c803 と c804 の写真が入れ替わっていた。**
    #    ⑤c-3 の注は `7997301`（srm_horizontal）を「砂漠の試験台に横たわる全尺モーター」と
    #    書いていたが、⑤c-5 が原寸で、⑤c' が元画像で確かめた**絵はその逆**：
    #      `7997301`（1979）＝**地面を横に走る炎と巨大な噴煙**＝燃焼試験そのもの
    #      `8776671`（1987）＝**砂漠の試験台に水平に置かれた全尺モーター**（燃えていない）
    #    → 写真を入れ替え、**年も一緒に動かす**（年は写真に付いている）。
    #      [[feedback-inventory-is-not-usable-material]]（絵が正本。題名も注も当てにしない）
    # ⚠️ 1979年＝**事故の前**の撮影。委員会が問うた当時のやり方と年が合う。
    # ⚠️ `bias=0.95`：既定の 0.5 だと、覆いが下 371px を落として**炎が字幕帯の下**
    #    （出力 y 1003〜）に沈み、画面は灰色の煙だけになる。⑤c-5 が c804 で見たのはこれ。
    #    実測＝元画像 3000x2430・覆い z=0.640・炎の上端は元 y1939。
    #    bias=0.95 で出力 y790 ＝ 字幕帯の上端 892 より上に出る。
    "c803": dict(
        t="問われたのは、試験のやり方",
        s="固体ロケットの燃焼試験　1979年・ユタ州",
    ...
    # 🔴🔴 2026-09-22 ⑤c'（10-5）：c803 と入れ替えた（上の注）。**この章でいちばん絵が効く**。
    #    `8776671`（1987）＝砂漠の試験台に**水平に置かれた全尺モーター**＝副題そのもの。
    # ⚠️ **1987年＝事故の後の撮影**。試験台に横たえる形は当時と同じだが、年は副題で名乗る
    #    → [[feedback-subtitle-must-match-what-is-visible]] / [[feedback-fallback-stills-must-match-the-era]]
    # ⚠️ 胴に `Thiokol` の英字（約18px）が写る。台本は「作った会社」と役で呼ぶが、
    #    社名は報告書が公に名指ししているので絵に残してよい（⑤c' 判断）。読める級数でもない。
    "c804": dict(
        t="飛ぶ向きと、試す向きが違った",
        s="試験台に横たわる補助ロケット　1987年・ユタ州",
