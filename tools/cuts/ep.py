...
SPEC = {
    ...
    # 🔴🔴 2026-09-20（⑤c'）：**「四つの段階」と言いながら、絵は三つだった。**
    #    真因は段の順番。`people` の段は「矢印を全部 → 節を全部」なので
    #    節5・矢印4＝**段9**になり、尺 10.5秒に対して
    #    4つめの節「維持管理」は **9.36秒に出はじめ、描き終わるのが 10.53秒**
    #    ＝**カットの終わり（10.5秒）に間に合っていなかった**。
    #    しかも矢印は段0〜3で先に引かれるので、**行き先の無い矢印**が長く出ていた。
    #    ＝ 9本目 c718（[[feedback-qa-still-is-mid-animation]] の逆で、**本物の粗**）と同じ型。
    #    → ① 中心の「一つの事故」を**先頭**に置く（矢印の行き先が先に立つ）
    #      ② `pair=True` で**節と、その節から出る矢印を1つの段**にする（段 9 → 5）
    #    ⚠️ x・y は1つも動かしていないので、**置き場所と大きさは前と同じ**。
    "ep07": dict(
        ...
        s="判決が使った罪名",
        fig=("people", dict(
            nodes=[dict(x=0.50, y=0.50, t="一つの事故", d="共同正犯", c=J.ALERT,
                        kind="doc"),
                   dict(x=0.14, y=0.24, t="計画", d="", c=J.INST, kind="person"),
                   ...
                   dict(x=0.86, y=0.76, t="維持管理", d="", c=J.AMBER,
                        kind="person")],
            edges=[dict(a=1, b=0, t=""), dict(a=2, b=0, t=""),
                   dict(a=3, b=0, t=""), dict(a=4, b=0, t="")],
            pair=True,
            note=SRC_DAE96)),
    ),
