...
SPEC = {
    ...
    "ep04": dict(
        ...
        s="箱に書き添えられた言葉（p191）",
        photo=ss.P191, side="right", ann_y=330,
        # 2026-09-06（6）🔴 注記は「1つめ」と「2つめ」の両方を指しているのに、
        #   **箱2の行が右端で 49〜220px 欠けていた**（k=0/k=1）。箱1と箱2（原画 x 130〜1241・
        #   y 493〜834）が両端で丸ごと入り、ほかの箱と下の一文は1行も入らない値。
        #   ⚠️ ep02・ep03 は同じ検査で欠け 0 件なので触っていない。
        xbias=0.06, bias=0.30, zoom=2.15,
    ...
    "ep05": dict(
        ...
        s="p191 の波括弧",
        photo=ss.P191, side="right", ann_y=330,
        # 2026-09-06（6）🔴 焼いた絵を見て決め直した。G-10/G-13 を数字で追ったせいで
        #   **箱1・箱2の8行が見出し帯に潜り、下の一文が画面を占めて字幕とぶつかって**いた。
        #   このカットが見せるのは「波括弧が左の2つをくくっている」ことだけ。条件は3つ：
        #     ① 箱1・箱2（原画 y 493〜834）が両端で丸ごと・見出し帯（画面 y<210）の外
        #     ② 波括弧（**画素で実測** 原画 x 73〜1270・y 925〜1022。OCR には映らない図形）が枠内
        #     ③ 下の一文（原画 y 1057〜1355）は1行も入れない
        #   この3つを満たす切り方を総当たりで出した（307通りのうち粗が最少のもの）。
        xbias=0.03, bias=0.343, zoom=2.0,
