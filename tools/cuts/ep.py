...
SPEC = {
    ...
    "ep03": dict(
        t="試し方も、書きこまれた",
        # 🔴 2026-09-22 ⑤c'（10-9 の11）：⑤c-5 が原寸で見た絵は**円筒をクレーンで
        #    吊り込む場面で、燃えていない**。副題は写っているものに合わせる
        #    （[[feedback-subtitle-must-match-what-is-visible]]）。
        s="新しい試験台へ吊り込む筒　1988年・マーシャル",
    ...
    "ep05": dict(
        t="外に、見る目を置く",
        # 🔴 2026-09-22 ⑤c'（10-9 の11・10-10 の3）：⑤c-5 は「絵は白衣の4人が
        #    のぞきこむ場面で公聴会でない／私人か公的任務か絵から決まらない」と挙げた。
        #    元の題は `PRESIDENTIAL COMMISSION - STS-33/51L - KSC`＝**大統領委員会が
        #    ケネディ宇宙センターで現物を確かめている場面**で、写っているのは
        #    **公的な任務の人**（[[feedback-jiko-photo-people-policy]]＝顔は可）。写真は替えない。
        # → 直すのは**副題だけ**。「公聴会」は写っていないので名乗らない。
        s="委員会による現場の確認　1986年",
    ...
    "ep10": dict(
        t="名前を、もう一度",
        # 🔴 2026-09-22 ⑤c'（10-9 の15）：⑤c-5 は「額が 645px で中央に小さく、
        #    下半分の人の頭が白飛び」と挙げた。元画像は 3000x3016 の**ほぼ正方形**で、
        #    `ss.kind` が縦横比 1.0（< PANEL_AR 1.12）で額装に回している＝それ自体は設計どおり。
        #    白く飛んでいるのは**下から4分の1**（y 0.69〜0.94・実測で 235以上が 1.6〜2.0%。
        #    手前にかぶった白いぼけ）なので、`trim` でそこを落とす。
        #    切ったあと 3000x2051（1.46）＝ 額は 645x648 → **948x648** に広がる。
        trim=(0.0, 0.0, 1.0, 0.68),
        s="追悼式　ヒューストン・1986年",
    ...
    "ep11": dict(
        t="生中継で、二回の予定だった",
        # 🔴🔴 2026-09-22 ⑤c'（10-7）：`crew_portrait_3` は c216 の `crew_whiteroom` と
        #    **最大差 1 ＝ 同じ写真**（NASA が別の題名で2度出している）。
        #    第2章の終わりとエンディングで同じ絵が出ていたので、ここを差し替えた。
        # ⚠️ 副題は**写っているもの**に合わせ、人物の名は名乗らない
        #    （誰が写っているかは元の説明からは確定できない）。
        s="訓練飛行から戻る乗員　1986年1月8日",
        photo=P("crew_flightline"), **ss.kind(P("crew_flightline")),
        bias=0.45,
    ),
