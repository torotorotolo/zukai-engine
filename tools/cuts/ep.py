...
■ 写真9カット（台本の画の欄どおり）＝オランダの追悼（Anefo・1977年）と国際追悼碑。
  ⚠️ 私人の顔が主役の写真は `NG_PHOTOS`（使うと止まる）。ep03 の葬列（`nl_funeral_02`）は前列の顔が
     判別できる大きさだった → ⑥でカズヤくん判断により `nl_funeral_06`（献花のあとの市長）へ差し替えた。
...
SPEC = {
    ...
    # 🔴 2026-09-17（⑥）カズヤくん判断：`nl_funeral_02` は前列の私人の顔が本人と分かる大きさ
    #    → 同じ連作の `nl_funeral_06` へ差し替え。副題は出どころの題名
    #    「Burgemeester Ivo Samkalden van Amsterdam na de kranslegging」どおり（写真→写真＝割合は不変）
    "ep03": dict(
        t="30秒足らずに、止まる機会が四つ",
        s="献花のあとのアムステルダム市長　1977年撮影",
        photo=P("nl_funeral_06"), bias=0.50, side="right", ann_y=356,
        **ss.kind(P("nl_funeral_06")),
        ann=[],
    ),
