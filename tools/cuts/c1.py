...
SPEC = {
    ...
    "c105": dict(
        ...
        fig=("punch", dict(
            stage=1, lead="プールサイドの足の下",
            zones=("上：プールデッキ（人・土・舗装）", "下：駐車場（車）"),
            note="模式図。寸法は書いていない")),
    ),
    ...
    "c112": dict(
        t="1.2センチで、扉は動かなくなる",
        s="門の札（p50）",   # 引用の「いつ」は札が持つ＝副題に書かない（ルール §1）
        fig=("quote", dict(
            phrase="門が開かなくなった",
            rows=[("誰が", "NIST（証言の描き起こし）", J.INST),
    ...
    "c113": dict(
        ...
        **ss.focus(ss.P050_GATE, 0.46, 0.40, 1.6),   # A-07: 下半分が地の空白＋矢印が見出しの裏だった
        ...
        fig=("compare", dict(
            items=[dict(v=12, t="門の沈み（3週間前）", disp="1.2", unit="cm",
                   ...
                   dict(v=4, t="扉と枠の隙間", disp="数", unit="mm", sub="扉は数ミリで作られる",
                        c=J.LINE)],
            vmax=30, ref="枠の高さ＝3 cm",   # A-08: 「人が気づく目安 3cm」は出どころが無いので枠の寸法だけを言う
    ...
    "c117": dict(
        ...
        s="NIST のまとめ（p62）",
        photo=ss.P062, side="left", ann_y=330, color=0.4,
        **ss.focus(ss.P062, 0.39, 0.30, 1.3),   # A-11: 吹き出しが見出しの裏＋1週間前の吹き出しが割り込んでいた
    ...
    "c121": dict(
        t="門の絵は、2段で終わらない",
        s="門の描き起こし　1週間前（p52）",
        photo=ss.P052_GATE, side="left", ann_y=600, color=0.5,   # A-14: 注記を3段目の横へ
        **ss.focus(ss.P052_GATE, 0.55, 0.78, 1.5),
        ann=[dict(t="3段目", d="矢印が、また下を向く（~1\"）", dc=J.ALERT, ds=30)],
    ),
    ...
    "c123": dict(
        ...
        photo=ss.P052_GATE, side="left", ann_y=330, color=0.5,   # A-15: 注記がキャプションの真上だった。白い柱の側へ
    ...
    "c125": dict(
        ...
        **ss.focus(ss.P052_GATE, 0.45, 0.78, 2.0),   # A-16: 2段目に寄っていた。語りは3段目（修理後）
    ...
    "c126": dict(
        t="書き添えは、ひとことだけ",
        s="門の3段目の札（p52）",   # 引用の「いつ」は札が持つ（ルール §1）
        fig=("quote", dict(
    ...
    "c127": dict(
        t="同じ週、駐車場のなかでも",
        s="NIST のまとめ（p62）",   # 注記 t「1週間前の吹き出し」と同語になるので副題から外す
        photo=ss.P062, side="left", ann_y=330, color=0.4,
        **ss.focus(ss.P062, 0.85, 0.75, 1.7),   # A-17: 吹き出しの最終行が字幕帯にかかり、3週間前の尻尾が見出しの裏だった
        ann=[dict(t="1週間前の吹き出し", d="門の損傷、ふたたび", dc=J.ALERT, ds=30),
             dict(t="", d="見られたのは、駐車場のなか", dc=J.LINE, ds=30)],
    ),

    # ── c128 見えていたのは水 ──────────────────────────
    "c128": dict(
        t="見つかったのは、水",
        s="崩落現場（NIST 記録映像 B-Roll #1）",   # A-18: 柱は写っていない（瓦礫と捜索隊）
