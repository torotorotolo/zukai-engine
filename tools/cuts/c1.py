...
SPEC = {
    ...
    "c109": dict(
        ...
        fig=("compare", dict(
            ...
            # 🔴 2026-09-17（⑤c' E-01）：`ref` は破線の枠の内側の上端に出る作り＝機長の棒の中に
            #    副操縦士の日付が入り、機長の棒の札に読めた。日付は note へ移す。
            vmax=1600,
            note="事故報告書 p7　副操縦士が747の資格を取ったのは1977年1月19日")),
    ),
    ...
    "c111": dict(
        ...
        **ss.kind(P("cockpit_747_02")),
        # ⚠️ 2026-09-17（⑤c B-04）：名前を写真の人物の頭の横に出すと本人の写真に見える
        #    （写真は1971年の別の便）＝名前は出さない（語りが言う）。役目は見出しが言うので、
        #    札は受け持ち（語りの「エンジンや燃料の装置を見張る」）にする（`check_wording` が
        #    「航空機関士」を見出しの答えの繰り返しとして止めた）。
        ann=[dict(t="受け持ち", d="エンジンと燃料", dc=J.INK_W, ds=40),
    ...
    "c118": dict(
        ...
        s="二機に乗っていた人の内訳",
        fig=("panel", dict(
            # ⚠️ 2026-09-17（⑤c B-03）：「14・234」の中黒が数字の書体で小数点に見えた＝人を付ける
            blocks=[dict(k="KLM機", t="乗員・乗客", v="14人・234人", c=J.AMBER),
                    dict(k="パンナム機", t="乗員・乗客", v="16人・378人", c=J.INK_W),
                    dict(k="補助席", t="航空会社の従業員", v="2人", c=J.OK)],
