...
SPEC = {
    ...
    "c406": dict(
        t="高さ一万四千メートル",
        # 🔴 2026-09-22 ⑤c'（10-7）：⑤c-5 は「c401 と目では同じ絵」と決着した。
        #    **差し替えの当てが無い**ことを確かめた＝NASA画像庫で `51-L`／`Challenger accident`
        #    ／`STS-51L` を引き直しても、未使用の事故の瞬間は `S86-38989` 1点だけで、
        #    それは `fireball_wide`（c401）と**最大差 1 ＝ 同じ写真**だった。
        #    図に替える手も、写真映像 45.5% の余裕が1カットしか無いので採らない。
        # → いまできるのは2つ。①副題を**写っているもの**に分け直す（c401＝広がった火の玉／
        #   ここ＝立ちのぼる柱と落ちていく筋）②寄りを変えて画の切り方を離す。
        # 🔴🔴 ⑤c'' で原寸を並べ、**2回出すか1回に減らすか**をカズヤくんに一度見てもらう。
        s="煙の柱と、落ちていく筋　1986年1月28日",
    ...
    "c412": dict(
        t="燃えたまま、飛びつづけた",
        # 🔴 2026-09-22 ⑤c'（10-9 の11）：原寸で見ても**筋は1本が主**で、2本目は
        #    x=1140 付近に細く出るだけ＝「二本」と名乗ると絵が数を裏切る。
        #    数は台本（ナレーション）が言うので、副題は**数えない言い方**にする。
        s="飛びつづける補助ロケット　1986年1月28日",
