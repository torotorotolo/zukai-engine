...
SPEC = {
    ...
    "c402": dict(
        t="報告書は、分けて書いている",
        s="起きたことの数えかた",
        # 🔴🔴 2026-09-22 ⑤c'（10-4）：`labels` を渡していなかったので、原寸で
        #    **灰色の丸3つと見出し一行だけ・札は0文字**だった。しかも `on=0` で
        #    3つとも沈んだ色＝「三つとも無い」と読める。
        # ⚠️ 札は**数だけ**（一つ目・二つ目・三つ目）。中身は c403／c405／c408 が
        #    順に明かすので、ここで書くと決め所が死ぬ（このファイルの冒頭 1.）。
        # ⚠️ `oncol` の既定は J.ALERT（赤）。ここは「報告書の数えかた」で
        #    危ないという話ではないので、ふつうの技術線の色に置く。
        fig=("icons", dict(
            n=3, on=3, kind="dot", cols=3,
            labels=["一つ目", "二つ目", "三つ目"], oncol=J.LINE,
            lead="報告書が分けた段階",
            note=SRC3)),
    ),
    ...
    "c405": dict(
        ...
        fig=("people", dict(
            ...
            # 🔴 2026-09-22 ⑤c'：c106 と**同じ形**（節4つ・矢印3本）。目視で出た1件を
            #    同じコードの別の枝へ広げた（[[feedback-scale-one-visual-finding-to-a-full-count]]）。
            #    ここは散る向きだが、`owner=max(a,b)` なので受け手といっしょに出る。
            pair=True,
            note=f"{SRC3}・{SRC4}")),
    ),
