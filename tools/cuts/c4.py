...
SPEC = {
    ...
    "c408": dict(
        ...
        s="入れた燃料で飛べたところ",
        fig=("mapfig", dict(
            # ⚠️ 2026-09-17（⑤c''）：ラスパルマスの札の左上に線の始まりが 1px で接した。札を下へ
            #    ずらすと添え書きが枠の下端に当たる＝二つの点を上へ 0.06 ずらす（向きは変えない）
            points=[dict(x=0.14, y=0.74, t="テネリフェ", d="ここで入れた", c=J.INK_W,
                         side="left"),
                    dict(x=0.26, y=0.82, t="ラスパルマス", d="入れ直さなくてよい",
    ...
    "c413": dict(
        ...
        s="上限の計算の決まり",
        fig=("timeline", dict(
            # ⚠️ 2026-09-17（⑤c' E-20）：t2「ここまでは簡単な計算」が日付と 2px で詰まり、
            #    帯の札「見る項目が少ない」と同じ行の高さで1行に読めた＝t2 を外す（帯の札と同じ話）
            events=[dict(t=1976.95, top="1976年12月", c=J.OK, big=True, up=True),
