...
SPEC = {
    ...
    # 🔴🔴 r10 の拡大目視：**c109 と同じ図を、同じ誤り方で使っていた。**
    #    r05 で c109（付図-23）は「飛行の記録」→「気象レーダーが見た雲」に直したのに、
    #    その隣の付図-24 を使うこのカットが**そのまま残っていた**。
    #    本文2.8.3：「レーダ・スケッチ図　**気象庁の**東京レーダー・スケッチ図
    #    （18時及び19時）は、付図‐23及び24のとおりである」＝**雲のエコー図**で、
    #    §2.8 気象に属する。実物の凡例も「S 7.0 Cu＝強い対流性エコー、高さ7.0キロメートル」。
    #    その手描きの線を、視聴者は**機影の航跡**と読む。しかも副題が原文にある
    #    「気象庁」を落としていたので、いっそう誤読を招いていた。
    #    → 写真を外し、本文2.14.1.1 の事実で組み直す。
    #      「同機の機影が18時57分に**羽田から磁方位308度59海里**の地点でレーダから消えた旨の
    #        通報を**18時59分**に東京アプローチから受領した」
    #    ⚠️ 消えたのは**管制**のレーダーであって、気象レーダーではない。
    "c417": dict(
        t="18時57分、機影が消えた",
        s="本文2.14.1.1　救難調整本部が受けた通報",
        # ⚠️ 報告書の表記は「レーダ」だが、この動画のナレーションと字幕は
        #    ずっと「レーダー」。同じ画面で2つの書き方を並べない（ルール §2-5）。
        fig=("moment", dict(clock="18:57", label="レーダーから消えた地点",
                            day=18.95, dayspan=(18, 24),
                            facts=[dict(t="羽田からの方位・距離", v="308度 59海里",
                                        c=J.LINE),
                                   dict(t="通報が届いた時刻", v="18:59", c=J.AMBER)])),
    ),
    ...
    "c421": dict(
        ...
        # 🔴 r10：その直しで、今度は `check_burned` が鳴った（**85%一致**）。
        #    この図は**題名が画像の中に焼き込まれている**（上端と、地図の右上の枠内の
        #    二か所）。しかも額の下の出典が「…解説書…図13／切出」を出しているので、
        #    副題に図名を書くと**画面に同じ題名が三つ**並ぶ。
        #    → 副題は「何を見ている図か」＝**この地図の読み方**を持たせる。
        #      公式の図名は出典欄と画像そのものが持っている。
        s="解説書 §9　①〜⑦が通報された位置",
    ),
