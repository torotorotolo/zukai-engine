...
SPEC = {
    ...
    "c403": dict(
        t="風を読む班",
        s="報告書の第17図　部隊の気象班",
        # ⑤c'（09-23）：下の辺が頁の番号「113」を横に切っていた＝図の注「Figure 17.」ごと外し、いちばん下の箱（ENEWETAK…）は寄りの終わりまで入れる（台帳 §7-4）
        photo=ss.page(118), panel=True, trim=(0.08, 0.455, 0.95, 0.916),
        side="right",
    ...
    "c418": dict(
        ...
        # ⑤c'（09-23）：寄りの終わりで図の注「Figure 55.」が窓の下の辺に切られていた＝注は副題が名乗るので外す。経度の数字は寄りの終わりまで入る（台帳 §7-4）
        photo=ss.page(214), panel=True, trim=(0.01, 0.05, 0.99, 0.80),
        side="right",
