...
SPEC = {
    ...
    "c901": dict(
        t="よく、こう言われている",
        s="跡地に残った管理棟 ARA-613",
        # 🔴 2026-09-07（⑤c' K-19/L-20）：**ネガの縁を窓の外へ出す。**
        #    haer_15 は右 x3487〜（原画3840）が真っ黒のフィルム縁で、そこに白い
        #    ネガ番号が縦書きで焼き込まれている（画面 x1750〜1884・全高）。
        #    ⚠️ 同じ番号は出典行にも書いてある＝二重。
        #    式＝cw = 3840/zoom。zoom 1.13 で cw 3398.2、xbias 0.10 で左 44.2 →
        #      窓の右端 3442.4 ＜ 3487（余裕 44.6px）。k=1 はさらに内側に寄る
        photo=ss.HAER_15, bias=0.5, zoom=1.13, xbias=0.10, side="right", ann_y=340,
    ...
    "c902": dict(
        ...
        s="この章で当たった一次資料",
        fig=("process", dict(
            # 🔴 2026-09-07（⑤c' L-15）：4列の工程図は1列が 366px＝**折り返し7字**しかなく、
            #    「AEC 調査委員会報告（1961年6月）」は**どう折っても語の途中で割れる**
            #    （式で先に出した。旧＝「報告（1961年6月／）」で「）」が行頭）。
            #    "AEC " を落とせば「調査委員会報告／（1961年6月）」で割れない。
            #    AEC はすぐ隣の 2 の欄と c904 の出典に出るので、意味は落ちない。
            steps=[dict(t="1", d="調査委員会報告（1961年6月）", c=J.DOC),
        ...
        fig=("compare", dict(
            items=[dict(v=0.01, t="駆動装置で入る速さ", disp="0.01",
                   ...
                   dict(v=20, t="手で速く引いた場合", disp="20", unit="%/秒",
                        sub="見積り", c=J.ALERT)],
            # 🔴 2026-09-07（⑤c' K-06/L-14）：**棒をやめた。**値の比は 1:2000 だが、
            #    棒の下限（4.5%）に張り付いて絵は 1:16.5 にしかならず、0.01 の棒は
            #    高さ 20px の細い線＝「量」として読めなかった（原寸で確認）。
            #    ⚠️ 台帳の当たりは `ratio="2000倍"` も足せ、だったが**採らない**。
            #      「2000倍」は**次の c915 の決め所そのもの**（台本「2000倍になりうる」）で、
            #      ここで先に出すと決め所が死ぬ（[[feedback-quote-phrase-timing]]）。
            #      c914 の台本はまだ 0.01 の話しかしていない。→ ② を優先した。
            bar=False,
