...
SPEC = {
    ...
    # コメントの問い（2026-09-24・⑥ 試写でカズヤくん指示＝13本目 c920 と同じ作り・文言は案1）。
    # 冒頭の c109「確かめたいのは、二つ」と対の形（問い1・問い2 の2列）。字幕と同じ文は画面に出さない（check_echo）
    # 🔴 2026-09-24（⑥-2 r05 の原寸）：問い2 の v を「、」で区切ると **Dela のまま 48px** で焼けた
    #    （`numfam` は字幅が Dela と Noto で同じ文字列だけ Noto へ落とす。「、」は Dela 0.414／Noto 1.0）。
    #    問い1（Noto 55px）と書体がそろわず、漢字がつぶれる型（c435）＝区切りは全角スペース（両方 1.0）
    "c921": dict(
        t="あなたは、どう考えますか",
        s="よければ、コメント欄へ",
        fig=("panel", dict(
            blocks=[dict(k="問い1", t="どこで防げたか", v="被曝を止められた所", c=J.AMBER),
                    dict(k="問い2", t="どちらに納得したか", v="語られる話か　報告書か", c=J.ALERT)],
            cols=2)),
    ),
