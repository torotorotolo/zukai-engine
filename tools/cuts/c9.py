...
SPEC = {
    ...
    "c902": dict(
        ...
        # 🔴 2026-09-22 ⑤c'（10-6）：吊り上げているかけらの `United` が**文字高 約100px で
        #    画面の中央**にあった（c113 と同じ語が2回）。台帳の当て＝「寄せか別の写真」。
        #    置き場に予備の写真は無い（87点すべて当たっている）ので**寄せで外へ出す**。
        # ⚠️ 寄りの下限は実測で決めた＝英字は元画像の x 0.30〜0.42 にあり、
        #    xbias=1.0 のとき左端が 0.42 を越えるのは zoom≧1.725。余裕を見て 1.8（左端 0.444）。
        # ⚠️ bias=0.45 で y 0.266〜0.675＝**作業する人と、覆いを掛けたかけらの列**が残る。
        #    拡大 1.71倍（元 2018px なので耐える）。⚠️ 🔴 ⑤c'' で原寸を見て決める。
        bias=0.45, xbias=1.0, zoom=1.8,
    ),
