...
# ── EL_YOMI（8本目 コロンビア号）───────────────────────────────
# 🔴 **空から始める。** 台本第2版 §6 の「誤読の危険がある語」は**危険の候補**であって、
#    読みの正解は入っていない。1語ずつ鳴らして（el_ab_yomi）確かめた語だけを、根拠を1行添えて足す
#    （feedback-yomi-dict-must-be-verified＝辞書が誤ると検査は全段素通りする）。
# 🔴 **7本目 9.11 の56件も引き継がない。**声は同じ Koichi のままだが、
#    **同じキーでも当たる行が変われば結果が変わる**（7本目の実測＝「ラングレー機の」は直り
#    「ラングレー機が」は同じカナ化で 1.000 → 0.816 に**壊れた**）。旧辞書は git の `27606de`。
# ⚠️ **キーは入れる前に `python tools/el_script.py --hits` で当たる行数を数える。**
#    1キーで複数行を直そうとすると、元は正しかった行を壊す。
# ⚠️ この回の申し送り（④'）:
#    ・**「マッハ」は本文から全廃した**（速さは全部「音の◯倍」）＝**辞書に入れない**
#    ・**FREESTAR も本文から落とした**＝入れない
#    ・「前のふち」は「前縁（ぜんえん）」に戻さない（表記はこれで揃えてある）
EL_YOMI = {
}
