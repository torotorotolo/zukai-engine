...
EL_YOMI = {
    ...
    "崩落のときに": "ほうらくのときに",
    # c516-1 文頭「崩落の25年前は」。取り直し4テイク目で「崩落」と聞き取れたテイクを、全行照合でもう一度かけると
    #    「風俗の」＝Scribe が割れる（音が弱い）。c327-2 と同じくかなにして取り直す。
    "崩落の25年前は": "ほうらくの25年前は",
    # c228-2「…4分から5分前。分の単位になる。」。聞取「十分の単位になる」で、語の時刻を見ると「十」に 0.36秒の実音がある
    #    ＝句点のあとの「分」を「じゅっぷん」のように読んでいる。読みを「ふん」に固定する。
    "分の単位になる": "ふんの単位になる",
}
