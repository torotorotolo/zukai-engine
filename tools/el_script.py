...
# 🔴🔴 **6本目キー橋の29件は引き継がない。**声が Hiro → **Koichi**（`H8ZPDxbrPcks5hEsi2fq`）に替わり、
#    stability も 0.85 → 0.64 に下がっている＝**別の声の実測は当てにならない**。旧辞書が要るときは
#    git の 2ea12fc（tools/el_script.py）から読む。⚠️ 短いキーの一括置換は、登録した回では正しくても
#    別の回の別の文脈で誤読を作る（「型」→「直感かた」の型）。
EL_YOMI = {
}
