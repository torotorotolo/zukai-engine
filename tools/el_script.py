...
EL_YOMI = {
    ...
    # 🔴 2026-09-21 試写：Otani だと**イントネーションが不自然**（2:10）。かな書きが平板さの原因なので
    #    素の漢字に戻す。⚠️ Sho のときは素だと『行動』に聞こえたので入れた語＝**戻したら聞取で確かめる**
    # "報道": "ほうどう",               # pr10-3（Sho の聞取『行動』）＝Otani では外した
    ...
    # 🔴 2026-09-21（声を Otani に替えた⑥）：Sho では「行方不明者の数」と読めていたが、
    #    Otani は **3回とも『若武者／武家武名者』**。半角空白だけでは足りないのでかなにした。
    "行方不明者の": "ゆくえふめいしゃ の",       # c811-1（Sho: 行方不明者の数 → Otani: 若武者数）

    # ── D. 🔴 声を Otani に替えて**新しく出た**誤読（2026-09-21・⑥）────────────────
    #    ⚠️ どれも Sho では正しく読めていた行。Scribe を**3回**起こして再現したものだけを入れる
    #       （1回の聞取で決めない＝`[outro jingle]` は3回起こし直したら正常だった実例がある）。
    #    ⚠️ 460行を Sho と突き合わせると **悪化35行・改善1行**、85%未満が **27→49行**。
    #       下はそのうち「読みそのものが違う」もの。残りは Scribe の表記ゆれ（cm・漢数字・ヒビ）。
    "梁": "はり",                      # 🔴 8行。Sho は全行 梁/張り。Otani は**文頭の梁**が
                                       #    c501-2『挙』／c503-1『ひょう』／c504-2『ひよ』／
                                       #    c506-2『ヒョ』／c711-1『筋』＝用語集に載せた語が崩れる
    "360キログラム": "サンビャクロクジュッキログラム",  # 🔴🔴 c415-2。**3回とも『160kg』＝数が違う**
                                       #    （Sho は『三百六十キログラム』）。いちばん危ない型
    "326人": "サンビャクニジュウロクニン",      # 🔴 ep13-3（3回とも『三比宇二十六人』）
    "64人をいったん": "ロクジュウヨニンをいったん",   # 🔴 c904-2（聞取『路地住四人』）。
                                       #    ⚠️ キーを「64人」にすると c914-1 の「4**64人**」に食い込む
    "設計に無い": "せっけいに無い",            # 🔴 c418-1（聞取『石鹸にない』）。
                                       #    ⚠️ 設計は10行に出るが崩れるのはこの行だけ＝この行だけに当てる
    "行き場を失って": "いきばを失って",          # 🔴 c705-2（聞取『浮き場を失って』）
    ...
    # 🔴 2026-09-21：表記を「7万1,136」→「71,136」に直したのでこのキーは当たらなくなった。
    #    門番が「台本のどの行にも当たらない」で止めてくれた（黙って素通りしない）。
    #    新しいキーは下の F 節の `"71,136"`＝**c304-2 と c306-1 の両方**に当たる。
    # "ときは7万1,136": "ときはナナマンセンヒャクサンジュウロク",   # c306-1 聞取『七万一千三百三十六』＝71,336
    ...
    # 🔴 2026-09-21 試写：Otani は「前」を**ぜん**と読んだ（5:45）。「まえ」まで送る
    "その30分前": "その サンジュップンまえ",                     # c115-2 3周のうち2周が『三分前』＝30分→3分

    # ── E. 🔴 試写（2026-09-21・カズヤくんの耳）で出た読みの誤り ──────────────
    #    ⚠️ ここは**耳の指摘が先**で、Scribe は裏取りに使う（[[feedback-ear-beats-the-meter]]）。
    #    ⚠️ 数は「その行だけ」でなく**同じ数が出る全行**に当てる（片側だけ直すと粗が隣へ移る）。
    "7月27日": "シチガツニジュウシチニチ",       # 8:42 c213-1（＋c819-1 も同じ数＝両方に当てる）
    "1994年": "千九百九十四年",               # 8:44 c213-2（＋c203-3）。⚠️ カタカナだと
                                       #    『1994年、根には』＝ネン+には が濁る。502人と同じく漢数字で通す
    "三豊建設": "サンプンけんせつ",             # 10:42 c305-2・13:02 c316-1「三豊」と「建設」が切れて聞こえる
                                       #    ⚠️ `三豊`→`サンプン` より**長いキー**なので先に当たる（4行）
    "次の一行": "次の いちぎょう",              # 11:22 c308-1（『いちごう』と読んだ）
    "細い柱": "ほそい柱",                    # 22:40 c522-1（『こそいはしら』に聞こえる）
    "男女の別": "だんじょの別",                # 35:59 c910-1（別の語に聞こえる）
    "31歳から": "サンジュウイッサイから",          # 35:31 c908-1
    "大法院": "だいほういん",                  # 38:38 ep06-2（＋ep04-2・ep08-1 も同じ語＝3行）
    # 🔴🔴 書き方を4つ実測して決めた（2026-09-21・`pr03-1` と `c907-1` の2行で A/B）:
    #      ゴヒャクニニン … 0/2『五百人』＝**に＋にん が潰れる**
    #      ゴヒャクニ 人  … 0/2『5200頭』『五百人人』＝空白で切ると**もっと壊れる**
    #      ごひゃくに人   … 0/2『500に人』
    #      **五百二人（漢数字）… 2/2 ✓** ＝これを採る
    #    ⚠️ かな化＝正解とは限らない実例（[[reference-elevenlabs-tts]]）。⑤a の「03分→漢数字」と同じ型。
    # 🔴🔴 2026-09-21 **r05 の試写（0:24 pr03-1）で、その「五百二人」が差し戻された**。
    #    カズヤくんの耳＝「別の語に聞こえる」。原因＝**「二人」は読みが2つある字（ににん／ふたり）**で、
    #    Otani は **ごひゃく"ふたり"** と読んでいた。
    #    ⚠️ **Scribe では絶対に見つからない型。**誤読でも同じ漢字「五百二人」と書くので、上の A/B は
    #       2/2 ✓ に見えていた（[[feedback-scribe-writes-the-right-word-for-misreadings]]）。
    #       ＝**上の4通りの A/B は「Scribe が判定した」だけで、耳では確かめていなかった。**
    #    → pr03-1 の本文まるごとで4通り鳴らし直し、**耳で** B を採った（2026-09-21・⑥試写）:
    #         A 五百二人 （r05 の音）… ごひゃくふたり
    #         B **五百二にん** … ✅ 採用。字幕「502人」も数え方「人」も変えずに読みだけ固定できる
    #         C 五百二名  … 崩れないが、聞こえる数え方だけ「名」にずれる
    #         D 502にん   … B の控え
    "502人": "五百二にん",                   # 0:24 pr03-1・35:03 c906-1・35:51 c909-2 ほか**6行すべて**
    "その502という": "その ゴヒャクニ という",       # 33:53 c822-2（裸の 502）

    # ── F. 🔴 表記を「N万N,NNN」→「NN,NNN」に直した数（2026-09-21 カズヤくん指示）─────
    #    ⚠️ **表記を変えると読みも変わる。**下は試写で名指しされた3件。
    #       残り（76,500／15,397／73,877／19,000／68,808／67,100）は**焼いてから Scribe で確かめる**
    #       ＝類推で入れない（[[feedback-yomi-dict-must-be-verified]]）。
    "71,136": "ナナマンセンヒャクサンジュウロク",      # 10:29 c304-2・c306-1
    "2,069": "ニセンロクジュウキュウ",            # 11:04 c306-3
    "30,892": "サンマンハッピャクキュウジュウニ",     # 33:40 c821-2（旧表記「3万892」＝万がゼロを隠していた）

    # 🔴🔴 表記を変えた残り8件も**全部誤読になった**（2026-09-21 に焼いて Scribe で実測）。
    #    ＝**カンマ区切りを「千の位」として読めない**。旧表記の「万」が支えになっていた。
    #      76,500 →『七十六千五百』／15,397 →『十五千三百九十七』／73,877 →『七十三点八百七十七』
    #      19,000 →『十九千』／68,808 →『六十八点八百八』／67,100 →『六十七千一百』
    #      34,000 →『三十四千トン』／73,205 →『七十三年二百五』
    #    ⚠️ 類推で先に入れず、**焼いてから確かめた**（[[feedback-yomi-dict-must-be-verified]]）。
    "76,500人": "ナナマンロクセンゴヒャクニン",                      # pr07-1・c209-1
    "15,397平方メートル": "イチマンゴセンサンビャクキュウジュウナナ平方メートル",     # c203-1
    "73,877平方メートル": "ナナマンサンゼンハッピャクナナジュウナナ平方メートル",     # c203-2
    "19,000平方メートル": "イチマンキュウセン平方メートル",                # c213-3
    "68,808人": "ロクマンハッセンハッピャクハチニン",                   # c807-1
    "67,100平方メートル": "ロクマンナナセンヒャク平方メートル",              # c809-2
    "34,000トン": "サンマンヨンセントン",                        # c821-2
    "73,205平方メートル": "ナナマンサンゼンニヒャクゴ平方メートル",            # c306-2

    # ── G. 🔴 試写の直しを焼いたあとに**新しく出た**誤読（2026-09-21・Scribe で実測）────
    "国会の報告書が": "こっかいの報告書が",       # c308-1（聞取『北海の報告書』）。
                                       #    ⚠️ 既存の「国会の報告書は、完全」は c409-1 専用＝別に要る
    # 🔴 c522-1「重い屋上。細い柱。沈んだ鉄筋。」＝短い決め所の行。`おもい屋上` では
    #    **テイクによって『浴場』**（おくじょう→よくじょう）になった（3回とも再現した取りがある）。
    #    ⚠️ 空白で切る案（`おもい 屋上`）は**「重い。屋上。」と句点が増える**＝この行の刻みが壊れる。
    #    → 全部かなにして空白も入れない形が唯一きれいに通った。
    "重い屋上": "おもい おくじょう",           # c522-1（聞取『暗い屋上』『重い浴場』）＝意味が変わる
    "白書が最終": "はくしょが最終",             # c906-1（聞取『各所が最終』）
    "白書には、亡くなった": "はくしょには、亡くなった",   # c907-1（聞取『各所には』）。
                                       #    ⚠️ 既存の「白書には、図面」は c503 専用＝この行には当たらない
    "鉄筋の太さ": "てっきんの太さ",             # 22:07 c520-2（聞取『付近の太さ』）＝**行頭**で子音が落ちる型
}
