...
CRITICAL_EP = ("札", "継ぎ目", "ゴムの輪", "補助ロケット", "乗員", "勧告", "輪切り", "機体", "主エンジン")
...
EL_YOMI = {
    ...
    # ── 行頭で崩れ、**取り直し5テイクでも直らなかった**3件（2026-09-21・A/B で確かめて採用）────
    # 🔴 順番＝**まず `el_retake` で振り直し、5テイクとも落ちたものだけ**ここへ入れた。
    #    ⑤a の実測＝崩れ25行のうち **20行は取り直しだけで直った**（＝テイクごとのブレ）。
    #    辞書に入れたのは**残った5行のうち3行**だけ。1行は下の ⚠️ のとおり却下、1行は本文を直した。
    #    ＝ [[feedback-audio-yomi-zero]] の「正しく読めている語をかな化しない」を守れる。
    # ⚠️ キーは**その行だけに当たる長さ**にしてある（`--hits` で1行ずつ確認ずみ）。
    #    「継ぎ目」だけをキーにすると 33行に当たり、正しく読めている24行のアクセントまで壊す。
    "継ぎ目は気温": "つぎめは気温",     # c609-2 行頭。5テイクとも 月目／次目／二目。A/B 88.9%→90.9%
    "札が掛かって": "ふだが掛かって",    # c811-1 行頭。5テイクとも 傘／3／差。A/B 75.6%→87.0%
    "その札を掛けた": "そのふだを掛けた",  # c812-1 5テイクとも 里／ファダ／さて。A/B 90.9%→95.5%
    # ── 「輪切り」は2行しかないので**両方を同じ読みに固定**した（feedback-audio-yomi-zero）────
    #    c501-2 は取り直し3テイクでも落ち着かなかった（ワニ皮／バニラ／間切）。A/B 83.3%→88.9% で良化。
    #    ⚠️ pr07-2 は**いま正しく読めている行**だが、対照として同じ A/B に掛けて
    #       **一致率も聞取も1文字も変わらない**ことを確かめてから入れた（＝悪化しない）。
    "輪切りの筒を運び": "わぎりの筒を運び",        # c501-2
    "輪切りの筒をつないだ": "わぎりの筒をつないだ",  # pr07-2（対照。変化なしを確認ずみ）
    # ⚠️ 🔴 **`c504-1` の「継ぎ目では」→「つぎめでは」は却下した**（A/B 83.3%→**70.3%**の悪化）。
    #    ねらう語を間違えていた＝聞取の「継目」は**つぎめ＝正しい音**で、Scribe が送りがなを省いただけ。
    #    本当に崩れていたのは **`下端`→`桁`（けた）**。かなにしたら継ぎ目まで「スニメ」に壊れた。
    #    → 辞書ではなく**本文を直した**（`下端`→`下のはし`・対の `c504-2` の `上端`→`上のはし` も）。
    #    [[feedback-jiko-plain-language]] にも合う（中学生が分かるか）。
    #    ⭐ 教訓＝**一致率が上がった／下がったではなく、「どの語が崩れているか」を先に決める。**

    # ══════════════════════════════════════════════════════════════════
    # ⑥ 試写（2026-09-22・カズヤくんが r04 を通しで聴いた）で出た **40件**
    # ══════════════════════════════════════════════════════════════════
    # 🔴🔴 いちばん大きい発見＝**崩れは「語」ではなく「位置」で決まっていた。**
    #    同じ語でも **行頭だけが崩れ、行中は正しく読めている**：
    #      上下   🔴c518-3 行頭 ／ 🔴c903-2 行頭 ／ ✅c506-3 行中
    #      1985年 🔴c810-1 行頭 ／ ✅c201-2・c209-1・c605-1 行中
    #      まん中 🔴c309-2 行頭 ／ ✅c319-1・c904-1 行中
    #      札     🔴c814-2 行頭 ／ ✅c810-3・c813-2 行中
    #    ＝ 上の「行頭で崩れた3件」は**同じ型がまだ17行残っていた**ということ。
    #
    # 🔴 **数字は聞取では絶対に捕まらない**（Scribe は「1984年」をどう読んでも
    #    「1984年」と書き戻す＝10本目の「502人＝ごひゃくふたり」と同じ形）。
    #    ⑤a が「取り直し26行・辞書5件」で閉じられたのは、**機械の網が全部通ってしまった**から。
    #    → 門番 `tools/check_yomi_numbers.py` を新設した（送信文字列に生の数が残っていたら落ちる）。
    #
    # ⚠️ **鍵は「当たる行」と「当たらない行」の両方を見てから決めた。**
    #    ⑤a の「キーはその行だけに当たる長さに」だけを守ると、
    #    同じ語の別の出現が生のまま残る（＝「札」が3行取りこぼされた真因）。
    #    ここで鍵を絞れるのは、**カズヤくんが全編を通しで聴いて、崩れた行を名指ししている**から
    #    （＝推測で絞っているのではない）。→ [[feedback-kinsoku-needs-both-ends]]

    # ── 読みそのものが違う（1行しか無いので語をそのまま鍵にできる）────
    "はね返る": "はねかえる",          # 9:23.7 c304-2  「は寝返る」
    "側である": "がわである",          # 19:04 c519-2  「そばである」
    "何か月": "なんかげつ",            # 23:31 c619-2  「なにかつき」
    "長いあいだ": "ながいあいだ",       # 25:54 c711-1  「ながいだ」
    "火を噴いた": "ひをふいた",         # 29:14 c813-3  「きをふいた」
    "妨害された": "ぼうがいされた",      # 31:48 c908-3  行頭・別の語
    "2時間近く": "にじかんちかく",      # 23:38 c620-1  「ちかくん」

    # ── 同じ語が複数行にあるので、**崩れた行だけ**に当たる鍵にした ────
    "技術者は、ひとりも": "ぎじゅつしゃは、ひとりも",   # 0:38 pr04-3「じゅじゅつしゃ」。他8行は正常
    "物理学の博士": "物理学のはくし",                # 6:48 c206-1「はかし」。c205-1 は正常
    "彗星が太陽": "すいせいが太陽",                  # 8:10 c214-1。c213-3 は正常
    "まん中の大きな": "まんなかの大きな",             # 10:17 c309-2 行頭「まんちゅう」
    "現地で組む": "げんちで組む",                    # 15:48 c503-1「ぜんち」。c501-2 は正常
    "控えの人も": "ひかえの人も",                    # 7:42 c211-1「控えの」が丸ごと落ちていた
    #    ⚠️ 「下半分」は **2行とも崩れている**（12:44 c320-3「しと」／12:48 c321-1「しま」）ので
    #       語をそのまま鍵にする。台本にこの語は2行しか無く、どちらも直す側なので巻き添えは無い。
    "下半分": "したはんぶん",                        # 12:44 c320-3 ／ 12:48 c321-1
    "5倍速く": "5倍はやく",                         # 18:25 c516-3「さやく」
    "上下に3度": "じょうげに3度",                    # 18:51 c518-3 行頭「じょうし」
    "上下の両側": "じょうげの両側",                   # 30:58 c903-2 行頭・別の語
    "札のことは": "ふだのことは",                    # 29:24 c814-2 行頭「さつ」
    "札を記した": "ふだを記した",                    # 29:28 c814-3「さつ」
    "本体のかけら": "ほんたいのかけら",               # 31:20 c906-1 行頭「こんたい」
    "同じ形で": "おなじかたちで",                    # 33:25 ep03-2 別の語
    "7人の家族": "ななにんの家族",                   # 32:32 c913-2「にちにん」。他9行は正常

    # ── 数（🔴 ここは聞取で検証できない。かなに固定するしかない）────
    #    ⚠️ 小数点以下は**1桁ずつ**読む（報告書の書き方をそのまま声にする）。
    "1984年、別の": "せんきゅうひゃくはちじゅうよねん、別の",      # 6:39 c205-2「しよねん」。c206-2 は正常
    "1985年、別の": "せんきゅうひゃくはちじゅうごねん、別の",      # 28:31 c810-1 行頭「ごごねん」。他3行は正常
    "37秒から": "さんじゅうななびょうから",                      # 11:25 c315-2「ごじゅうななびょう」
    "58.788秒": "ごじゅうはちてんななはちはちびょう",             # 11:52 c317-1
    "64.660秒": "ろくじゅうよんてんろくろくぜろびょう",           # 12:22 c319-2
    "73.124秒": "ななじゅうさんてんいちによんびょう",             # 12:30 c319-3
    "73.137秒": "ななじゅうさんてんいちさんななびょう",           # 13:00 c322-1
    "73.618秒": "ななじゅうさんてんろくいちはちびょう",           # 13:10 c322-2
}
