...
SLUG = "ep8"                # 8本目 コロンビア号（2026-09-14）。7本目 "ep7"・6本目 "keybridge"・5本目 "sl1"・4本目 "surfside"
...
# ── EL_YOMI（8本目 コロンビア号）───────────────────────────────
# 🔴 **空から始めた。** 台本第2版 §6 の「誤読の危険がある語」は**危険の候補**であって、
#    読みの正解は入っていない。1語ずつ鳴らして（el_ab_yomi・72件）確かめた語だけを、根拠を1行添えて入れた
#    （feedback-yomi-dict-must-be-verified＝辞書が誤ると検査は全段素通りする）。
# 🔴 **7本目 9.11 の56件も引き継いでいない。**声は同じ Koichi のままだが、
#    **同じキーでも当たる行が変われば結果が変わる**（7本目の実測＝「ラングレー機の」は直り
#    「ラングレー機が」は同じカナ化で 1.000 → 0.816 に**壊れた**）。旧辞書は git の `27606de`。
# ⚠️ **キーは入れる前に当たる行数を数えた**（`qa_out/ep8_yomi_plan.py` の門番）。
#    実際に2件止まった＝「破片の位置」は無事な c305-1 にも当たる／「いちばん西で」は無事な c718-2 にも当たる。
# ⚠️ この回の申し送り（④'）:
#    ・**「マッハ」は本文から全廃した**（速さは全部「音の◯倍」）＝**辞書に入れない**
#    ・**FREESTAR も本文から落とした**＝入れない
#    ・「前のふち」は「前縁（ぜんえん）」に戻さない。⚠️ **聞取で12行が「前の縁」と出るが誤読ではない**
#      （送っているのは かな の「ふち」。janome が「縁」を エン と読んだだけ＝直さない）
EL_YOMI = {
    # ── 語そのものが崩れていた（複数行に当てる。**無事な行を陰性対照にして確かめた**）──────
    "管制": "かんせい",              # 関税／寛治／漢字 → 管制・完成・感性（音は カンセイ で正しい）。5行中4行が崩れ。
    #  ⚠️ **陰性対照 c705-2**（崩れていない行）に当てても聞取は1字も変わらなかった＝巻き添えなし
    "クレーター": "クレエタア",      # 🔴 Crater（英字）／クリエイター → クレーター。**3行とも崩れ**
    "見積り": "みつもり",            # 蜜／ミスり → 見積もり。2行とも崩れ
    "〜": "から",                    # 🔴🔴 『to』『と』と読んでいた。**「670と920」は別の意味になる**。3行とも崩れ
    # ── 文頭が別語になっていた（かな書きで句の頭を固定する）──────────────────
    "オービタの先": "オービタ の先",      # 大尾びた → オービタ（3行中この行だけ崩れ）
    "強化炭素": "きょうか炭素",           # 🔴 硫化炭素＝別の物質名 → 強化炭素
    "その値は": "その あたいは",          # その名帯 → その値は
    "高度は約": "こうどは約",             # 🔴 速度は → 高度は（意味が変わる）
    "上がっていく": "あがっていく",       # 🔴 曲がっていく → 上がっていく（意味が反転）
    "当たった範囲": "あたった範囲",       # 渡った範囲 → 当たった範囲。⚠️「当たった」は12行あるので**この行だけ**に絞った
    "断熱材の衝突": "だんねつざいの衝突",  # 🔴 環熱帯 → 断熱材。⚠️ 断熱材は16行あるので**この行だけ**
    "耐熱タイルより": "たいねつタイルより",  # 🔴 媒熱タイル → 耐熱タイル。⚠️ 6行あるので**この行だけ**
    "指揮系統を": "しきけいとうを",       # 🔴 不否認制度 → 指揮系統
    "塞ぐ材料": "ふさぐ材料",             # 探査具材料 → 塞ぐ材料
    "着陸で翼": "ちゃくりくで翼",         # 陸で翼（頭欠け）→ 着陸で翼。⚠️ 着陸は5行あるので**この行だけ**
    "飛行が終わって": "ひこうが終わって",  # 機構が → 飛行が。⚠️ 飛行は39行あるので**この行だけ**
    "班をまとめ": "はんをまとめ",         # プランをまとめ → 班をまとめ
    "通信の担当": "つうしんの担当",       # 球審の担当 → 通信の担当
    "飛行主任は": "ひこうしゅにんは",     # 機工主任 → 飛行主任
    "破片は、ケネディ": "はへんは、ケネディ",  # 馬券 → 破片。⚠️ 「破片は」は10行あるので**この行だけ**
    "破ったのは": "やぶったのは",         # ギャブった → 破った
    "安全を担当": "あんぜんを担当",       # 万全を担当 → 安全を担当
    "打ち上げを3": "うちあげを3",         # 🔴 仕上げを → 打ち上げを（意味が反転）。⚠️ 打ち上げは15行あるので**この行だけ**
    #  ⚠️ キーを「打ち上げを3方向」にすると **1行も当たらず、門番が止めた**。
    #     `yomi_pattern` は「漢字で終わるキーは直後が漢字なら当てない」＝“方向”の次の“以上”に塞がれる。
    #     数字で終わらせると境界は「次が数字でないこと」になるので当たる。**A/B で試したのと同じ音**。
    # ── 行の途中の語 ────────────────────────────────────
    "破片の位置が51": "はへんの位置が51",  # 馬 pen／バペン → 破片。
    #  ⚠️ キー「破片の位置」だと **c305-1（無事）にも当たる**ので「が51」まで足した
    "破片の影響": "はへんの影響",         # 派遣の影響 → 破片の影響
    "追悼の頁": "追悼の ぺーじ",          # 追悼のやじな → 追悼のページ
    "手続きを問うた": "手続きを とうた",   # 🔴 手続きを踏んだ → 問うた（意味が反転）
    "式が試された": "しきが試された",     # 知識が試された → 式が試された
    "乗り移る": "のりうつる",             # 🔴 海苔を釣る → 乗り移る
    "にくい降り方": "にくい おりかた",     # 掘り方 → 降り方。7本目にも同じ「の降り方」があった
    "地上から「降りてよい」": "ちじょうから「降りてよい」",  # 事情から → 地上から。
    #  ⚠️ キー「地上から」だと3行に当たり、うち2行（c218-1・c412-1）は無事＝**この行だけ**に絞った
    "機体は傾きを": "機体は かたむきを",   # 型抜きを → 傾きを
    "左の主脚の": "左の しゅきゃくの",     # 左の死脚の → 左の主脚の
    "場内の放送は": "じょうないの放送は",  # 🔴 「場内の放送は」が丸ごと消えていた
    "いちばん西で見つかった": "いちばん にしで見つかった",  # 一番にして → 一番西で。
    #  ⚠️ キー「いちばん西で」だと **c718-2（無事）にも当たる**ので「見つかった」まで足した
    "縦横40センチ": "たてよこ40センチ",   # 「縦横よろこ四十センチ」＝二重読み → 縦横四十センチ
    # ── 数（7本目の教訓＝崩れた数は固定する側に倒す）──────────────────
    "0.008ジュール": "れいてんれいれいはちジュール",  # 🔴🔴 0.08＝**10倍違っていた**
    "最初は1983年": "最初は センキュウヒャクハチジュウサンねん",  # 🔴🔴 1983 が丸ごと消えていた
    "2台の映像": "ニダイの映像",          # 次代の映像 → 二台の映像
    "19,700立方センチ": "いちまんきゅうせんななひゃく立方センチ",  # 🔴 十九千七百＝万の位が無かった
    "1月24日、チーム": "イチガツ24日、チーム",  # 🔴 「月二十四日」＝文頭の 1 が消えていた
    "255周目": "ニヒャクゴジュウゴ周目",  # 🔴🔴 百五十時十五週目＝桁が壊れていた
    "ここを0秒とする": "ここを ゼロびょうとする",  # 🔴 「ここを秒とする」＝0 が消えていた
    "425秒後には": "よんひゃくにじゅうご秒後には",  # 🔴🔴 225秒後＝**別の数**になっていた。
    #  🔴 2026-09-15 ⑥の試写：**数は合っているが片言になっていた**（カタカナ9モーラを
    #     一息で置いたため）。**ひらがなに替えて「秒後」は漢字のまま残す**＝直すのは
    #     数の並びだけで、続きの語の流れは壊さない。⚠️ 空白で切ると**さらに刻まれる**ので入れない
    "270の機関から": "ニヒャクナナジュウの機関から",  # 🔴 麻薬70の期間から
    # ── 2周目（1周目のかな化が効かなかった／悪化した8件。別の書き方で通った）────────
    #    1周目の教訓＝**かな化は悪化する側にも倒れる**（離陸→りりく が『リリック』／
    #    積み荷→つみに が『隅の』／計算→けいさん が『Kさん』／NASA→ナサ が『母』）。
    #    2周目は「かなにせず**アクセント句を半角空白で切るだけ**」を主に試した。
    "NASAは確かめ": "NASA は確かめ",   # 🔴 噂は → NASAは（100%）。カナ『ナサ』は『母』になった＝英字のまま空白で切る
    "15通りの経路": "ジュウゴ とおりの経路",  # 🔴 十も通り → 十五通り。カナ＋空白の両方が要った
    "チタンの小片": "チタンの しょうへん",    # 小刀 → 小片
    "8枚目の下半分": "8枚目の したはんぶん",  # 「八枚目の一半分」→ 下半分
    "約4,400度": "約 ヨンセンヨンヒャク度",   # 「四四〇〇度」＝1桁ずつ読んでいた → 四千四百度。
    #  ⚠️ 一致率は 100%→95.5% に**下がった**が、下がったのは漢数字の表記のせい。**読みは直っている**
    #     （[[feedback-ear-beats-the-meter]]＝一致率で採否を決めない）
    "83,900点": "ハチマンサンゼンキュウヒャクてん",  # 🔴🔴 「八十千点九百点」→ 八万三千九百点。
    #  ⚠️ かな混じり「はちまんさんぜん…」は『八幡さん、1900点』に**悪化した**＝全部カナにして通った
    "1,052キロ": "センゴジュウニキロ",    # 🔴「見積り」を直したら『一点五十二』に化けた → 千五十二
    "号は、9時00分18秒": "号は、クジ ゼロフン ジュウハチビョウ",  # 「九時零零分」→ 九時零分。
    #  ⚠️ 1周目は「9時00分18秒」を**2行に**当てて pr02-2 は良化・**c725-1 は悪化**した。
    #     c725-1 はもともと「九時零分」で崩れていない＝**pr02-2 だけ**に絞る（7本目のラングレー型）
    # ── 3周目（**2周目の全文起こし**で見えたもの。1周目の聞取だけでは決められなかった）──────
    "RCCの板でいえば": "アールシーシーの板でいえば",  # 🔴🔴 **1周目も2周目も LCC**。
    #  ⚠️ **いったん「Scribe の揺れ」と判定して外していたのを取り消した。**A/B の1回だけ RCC と出たので
    #     揺れと見たが、**全文起こしは2周とも LCC**。1回の観測で決めない
    #     （[[feedback-verify-your-own-instrument]]）。RCC はこの回の中心の語（台本 §6 の筆頭）
    "予算とカメラ": "よさんとカメラ",       # 『ロサンと』→ 予算と（100%）。
    #  ⚠️ 予算は2行あり、**1周目は c905-1 が『助産』・2周目は c214-1 が『ロサン』**＝行は違うが語が弱い。
    #     c905-1 は2周目に直っていたので**当てていない**（正しく読めている行はかな化しない）
    "差の分だけ": "さの分だけ",             # 『浅野分だけ』→ 差の分だけ
    "軌道にいるコロンビア号の左": "きどうにいるコロンビア号の左",  # 🔴『非道にいる』＝意味が反転。
    #  ⚠️ キー「軌道にいる」だと **pr08-1（無事）にも当たる**ので「コロンビア号の左」まで足した
    "前のふち用の式": "前のふち用の しき",  # 『前のフチオの式』→ 前のフチ用の式
    # ── 4周目（**2026-09-15 ⑥の試写でカズヤくんが耳で見つけた13件**）──────────────
    #    🔴 ここは機械の聞取（Scribe）では取れないものが多い＝**耳が機械より上位**
    #       （[[feedback-ear-beats-the-meter]]）。「はぺん／はへん」「がいぱん／がいはん」は
    #       Scribe がどちらも「破片」「外板」と書くので、一致率では1件も鳴らない。
    #    ⚠️ キーは全部 `--hits` で **当たる行数を数えてから**入れた（7本目のラングレー型の事故を避ける）。
    "9時00分18秒。地上": "9時 ゼロフン ジュウハチビョウ。地上",
    "機体が分かれる": "きたいが分かれる",  # 🔴 取り直しの振れで『額(ひたい)が分かれる』に化けた
    #  ⚠️ r01 では正しく読めていた行＝**直しの巻き添えではなく、取り直しそのものの振れ**
    #     （stability 0.64 は表情が出るぶん振れも大きい＝el_script 冒頭の注記のとおり）。
    #     c518-3 でも同じ「機体→額」が出ている＝**この声はこの語が弱い**  # 🔴 27:45 c725-1。「00分」が崩れた。
    #  ⚠️ 2周目に pr02-2 だけへ絞った判断（c725-1 は当時は崩れていなかった）は**取り消していない**。
    #     既存キー「号は、9時00分18秒」はそのまま。**c725-1 用に別のキーを足した**
    "破片は、1秒に約18回": "はへんは、1秒に約ジュウハチかい",        # 10:11 c309-1。『はぺん』→ 破片。
    #  ⚠️ 「破片」は **25行**あるので当てない。崩れているのは指摘の2行だけ
    "破片がどんな形": "はへんがどんな形",   # 10:52 c313-2。同上
    "50分の1": "ゴジュウブンのいち",        # 13:36 c410-2。『ごじゅっぷんのいち』→ ごじゅうぶんのいち
    "に黙とうを": "に モクトウを",                   # 13:46 c411-2。🔴『だまとう』＝別語
    "三度とも": "さんどとも",                 # 16:09 c506-1。**行の頭**で語頭が飲まれる → かなで立て直す
    "機体の外板": "きたいの がいはん",                     # 18:17 c518-3。『がいぱん』→ 外板
    "15度を超えて": "ジュウゴドを超えて",   # 18:43 c520-2。「15」と「度」が分かれる → つなげる。
    #  ⚠️ 同じカットの c520-3「15通り」は既存キーで別に直してある（混ぜない）
    "一行も無い": "イチギョウも無い",        # 19:49 c527-1。違う語に聞こえる → いちぎょう。
    #  ⚠️ カズヤくんの指摘は 18:52 だが、**本編で「一行」が出るのはこの1行だけ**
    "2月10日から15日": "ニガツ10日から15日",  # 21:21 c609-2。**行の頭に異音**が入っていた。
    #  ⚠️ これは読み間違いではなく**合成の当たりはずれ**。送る字を変えると指紋が変わって
    #     **取り直しになる**ので、それを狙って頭をかなにした
    "空になった": "からになった",           # 21:42 c611-2。🔴『そらになった』＝意味が変わる
    "集まった破片": "あつまった破片",        # 32:03 c820-1。**行の頭**で語頭が飲まれる。
    #  ⚠️ 「集まった」は2行あるので「破片」まで足して c820-1 だけに絞った
    "38,500キログラム": "サンマンハッセンゴヒャクキログラム",  # 32:06 c820-1
    "84,900ポンド": "ハチマンヨンセンキュウヒャクポンド",      # 同上。
    #  ⚠️ 数は**全部カナ**にする（「83,900点」で、かな混じりが『八幡さん』に悪化した実績）
}
# 🔴 **入れないと決めた語**（次の回でまた疑わないために残す）:
#  ── ① 同じ音を2回起こしたら結果が変わった＝**Scribe の揺れ**（本編の全文起こしでは崩れたが、
#       A/B で同じ pcm を起こし直したら台本どおりだった。**音は変えていない**ので要耳へ）:
#     c208-2「RCC」… 全文起こしでは **LCC**。A/B の前後とも「RCC」。⚠️ RCC は8行あって他7行は無事
#     c709-1「290秒後」… 全文起こしでは **290 が消えた**。A/B の前後とも「二百九十秒後」
#     c503-2「誰が」… 全文起こしでは **彼が**。A/B の前後とも「誰が」
#     ep06-1「破片は今も」… 全文起こしでは **アペンは**。A/B の前後とも「破片は」
#     c905-1「予算の不足」… 全文起こしでは **助産の不足**。A/B の前後とも「予算の不足」
#     → [[feedback-verify-your-own-instrument]]／7本目の「2周とも崩れたものだけ直す」と同じ扱い
#  ── ② 読みは同じで字だけ違う（janome／Scribe の当て違い。**直さない**）:
#     前のふち（12行）… 聞取「前の縁」＝ まえのふち。送っているのは かな なので音は決まっている
#     そのあいだ→その間／このあと→この後／誰ひとり→誰一人／朝いっぱい→朝一杯 … janome の読み
#     破れる→敗れる（c520-2）… どちらも ヤブレル
#     記章→機章（c405-2）… どちらも キショウ
#     81.7秒→八十一点七秒 ほか（11行）… 小数点の「点」。読みは正しい
#  ── ③ かな化・空白で**悪化した**もの（2周目で別の書き方を試した。下の §2周目 を見る）:
#     離陸から→りりくから ＝『リリックから』／積み荷→つみに ＝『隅の』／計算→けいさん ＝『Kさん』
#     黙とう→もくとう ＝『供養』／NASAは→ナサは ＝『母は』／83,900点 ＝『八幡さん、1900点』
#     9時00分18秒 … pr02-2 は良化したが **c725-1 は悪化**（同じ文字列でも行で結果が逆＝7本目と同じ型）
