...
# ── EL_YOMI（7本目 9.11）─────────────────────────────────────────
# 🔴 **空から始めた。** 台本第2版 §6 の「怪しい語」は**怪しいだけ**で、読みの正解は入っていない。
#    1語ずつ鳴らして（el_ab_yomi）確かめた語だけを、根拠を1行添えて足している
#    （feedback-yomi-dict-must-be-verified＝辞書が誤ると検査は全段素通りする）。
# 🔴🔴 **6本目キー橋の29件は引き継いでいない。**声が Hiro → **Koichi**（`H8ZPDxbrPcks5hEsi2fq`）に替わり、
#    stability も 0.85 → 0.64 に下がっている＝**別の声の実測は当てにならない**。旧辞書は git の 2ea12fc。
# ⭐ 2026-09-13 ⑤a で確定した 36件。**すべて A/B（el_ab_yomi・55件）で実測してから入れている**。
#    判定の物差し＝`el_ab_verdict.py`（一致率では決めない。狙った音が後の聞取にだけ現れたか）。
#    記録＝audio/el_qa/ep7_yomi_ab.tsv・ep7_ab_verdict.tsv。当たりの出どころ＝ep7_reading_diff.tsv（79行）を
#    `qa_out/ep7_triage.py` で ①文頭45／②行の途中23／③読みが同じ11 に仕分けたもの。
# ⚠️ **境界規則が効いている**（複合語の内部には当てない）:
#    「管制」は 管制**官**（直後が漢字）にも 航空路**管制**センター（直前が漢字）にも当たらない。
#    だから「管制官」は別のキーで入れている。**この2件は必ずセットで見ること。**
EL_YOMI = {
    # ── 読みそのものが違っていた語（前の聞取 → 後の聞取）────────────────────
    "管制": "かんせい",              # 漢字江／漢字への／パンジエ／カン・ジエ → 管制・感性（音は正しい）。16/35行で崩れていた
    "管制官": "かんせいかん",        # 関税官／パンジエ艦／万事栄冠 → **管制官**。6/12行。⚠️ c220-2 は聞取から**語が丸ごと消えていた**
    "防空司令部": "ぼうくう司令部",  # 🔴🔴 **航空**司令部（意味が反転）／悟空スリーブ／Vocal Slave → 防空司令部。
    #  ⚠️ 全部かな「ぼうくうしれいぶ」でも 100% だったが、**漢字を残すほうを採った**
    #     （かなにしすぎると崩れる＝6本目「防衝工→王昭君」。司令部＝シレイブ は読みが1つなので安全）
    "報せ": "しらせ",                # どうせ／伏せ／むせ／夢癖／向くせ → 知らせ。**5行すべて崩れていた**
    "墜ち": "おち",                  # すいちる／ウツシル／移していた／ついちる → 落ちる（音は オチル で正しい）。墜 を ツイ と読んでいた
    "便名": "びんめい",              # 弁名／弁明 → **便名**（100%）。便＝ビン が ベン に寄る
    "北棟": "きたとう",              # 🔴🔴 北東／北道／北堂 → **北棟**。§6 の「ほくとう／きたとう どちらか」への答え。
    #  ⚠️ 「ほくとう」も A/B したが、**聞取が『北東』になった**＝この動画は方角（西南西・南へ・東の海上）が
    #     多いので、音読みは方角と取り違える。実測で きたとう を採る（c217-2・c313-1 とも「北棟」が出た）
    "南棟": "みなみとう",            # ⚠️ 単独では崩れていない（送っても聞取は変わらない＝害が無いことを実測）。
    #  北棟を訓読みに固定した以上、「きたとうと なんとう」は不揃いなので揃える
    "当直室": "とうちょく室",        # 盗聴室 → 当直室（100%）。室＝シツ は読みが1つなので後ろに残せる
    "客室": "きゃくしつ",            # 角質／学術 → 客室（100%・2行とも）
    "五角形": "ごかっけい",          # 御所系／五尺径 → 五角形（100%）
    "猶予": "ゆうよ",                # 🔴 ヨーヨー → 猶予
    "滑走路": "かっそうろ",          # 走路（カッが落ちた）→ 滑走路（100%）
    "誘導路": "ゆうどうろ",          # 流入道路 → 誘導路（100%）
    "分単位": "ふんたんい",          # 瞬単位 → 分単位
    "刃渡り": "はわたり",            # 🔴 「100あたり10cm」 → 刃渡り十センチ
    "首都の": "しゅとの",            # 使徒の方角／主島の別の → 首都（100%）
    "言葉": "ことば",                # 無線のことか → 無線の言葉（100%）
    # 🔴🔴 **1つのキーで2行を直そうとしたら、片方を壊した**（2026-09-14・⑥）。**行ごとに分ける。**
    #   `c708-1` … 「ラングレーき」だと聞取『ラングレー**、気**』＝⑥の試写でカズヤくんが指摘した「分かれて聞こえる」。
    #               カナで続けて `ラングレーキ` にしたら聞取『ラングレー機』（0.906→0.943）✅
    #   `c809-1` … こちらは「ラングレーき」で**一致率 1.000（崩れていない）**だった。
    #               同じカナ化を当てたら聞取『**ダングル駅**』0.816 ＝**直しが壊した**ので元に戻す。
    #               ⚠️ 辞書の「F-16 をかなにすると巻き添え（ラングレー→ダングレー）」と同じ型。
    #   → [[feedback-verify-the-ledgers-suggested-fix]]／[[feedback-scale-one-visual-finding-to-a-full-count]]
    #      （広げる先は同じでも、**当たる行ごとに結果が違う**）
    "ラングレー機の": "ラングレーキの",  # c708-1 だけ。⑥試写の指摘
    "ラングレー機が": "ラングレーきが",  # c809-1 だけ。**元の値のまま**（1.000 を壊さない）
    # ── 🔴 ⑥ の試写でカズヤくんが耳で拾った5件（2026-09-14）──────────────────
    #    🔴🔴 **読みの正解はカズヤくんの耳が決めた**（[[feedback-ear-beats-the-meter]]）。
    #    ここで決めるのは「その音を出す書き方」だけ。焼いたあと Scribe の聞取で当たりを確かめる。
    #    ⚠️ **キーは当たる行数を数えてから決めている**（`python tools/el_script.py --hits`）。
    "という年から": "という としから",   # 🔴「2001年という**年**から」が ねんから → **としから**。
    #  ⚠️ キーを「年から」にすると `c121-3`「1986年からの15年」`c311-1`「1970年から」まで
    #     とし に化ける（この2行は ねん が正しい）＝**当たるのが pr05-1 だけになる長さ**にした
    "飛んでいた機": "飛んでいた き",     # 🔴「機」が別の語に聞こえる → **とんでいた き**。
    #  ⚠️ 「飛んでいた」は正しく読めているのでかな化しない（アクセントが壊れる）。空白で切って 機→き だけ
    "時、分、秒の順": "じ、ふん、びょうの順",  # 🔴「左から時、分、秒の順」の3つとも別語に。
    #  ⚠️ 読点は**キーと同じ2つ**（値に足すと台本に無い間が入る＝③の門番）
    "それまで通っていた": "それまで とおっていた",   # 🔴 かよって → **とおって**
    "たびに通っている": "たびに とおっている",       # 🔴 同上（`c909-1`）
    # ── 文頭の頭欠け（かなで直った型。5本目 SL-1 の 建屋→たてや と同じ）──────────
    "軍の": "ぐんの",                # 屯の指揮系統 → 軍の（100%）。⚠️「軍が」は**振り直すと直っていた**＝入れない（下記）
    "犯人と": "はんにんと",          # 万人と戦う → 犯人と（100%）
    "点は": "てんは",                # 頭が丸ごと落ちた（聞取「はペンタゴンの…」）→ 点は
    "2棟は": "にとうは",             # 地頭は → 二棟が
    "7棟": "ななとう",               # 花塔の建物 → 七棟の建物（100%）
    "鳴った人": "なった人",          # だった人だけ → なった人だけ（鳴った＝なった。音は正しい）
    "符号が": "ふごうが",            # 時報が → 富豪が（ふごう＝音は正しい。Scribe の字の当て違い）
    "命令から": "めいれいから",      # 帰還から → 命令から
    # ── アクセント句を半角空白で切る型（かな単独では効かない／かなにすると別語になる）──────
    "の側": "の がわ",               # 空港の差 → 空港のガワ（音は正しい）。6本目と同じ型
    "この知らせ": "この しらせ",     # このチラシ → この知らせ
    "空の扱い": "そらの扱い",        # 全米のソロの扱い → 全米の空の扱い（100%）
    "いつ火が": "いつ ひが",         # 🔴 いつ木が（意味が変わる）→ いつ日が。**送るのは かな なので音は ひ で決まる**
    "離れた機も": "離れた 機も",     # 基地も → きも。⚠️ かな「きも」は 肝 に寄るので**空白で切るだけ**にした
    "飛ぶ機だけ": "飛ぶ きだけ",     # 基地だけ → 木だけ（き＝音は正しい）
    "落ちた機の": "落ちた きの",     # 落ちたきたの → 落ちた木の（き＝音は正しい）
    "ない便に": "ない びんに",       # 存在しない弁に → 瓶に（びん＝音は正しい）
    "の降り方": "の おり方",         # 着陸の掘り方 → 着陸の降り方
    "想定の外": "そうてい の外",     # 🔴🔴 童貞の底から → 想定の外から（100%）。
    #  ⚠️ かな単独「そうてい」では『造幣』になって直らなかった＝**空白でアクセント句を切る**のが決め手
    "ただしその": "ただし その",     # まだし → ただし（100%）。文頭の た が ま に落ちる
    # ── 数はカナ／かなで固定する（reference-elevenlabs-tts）────────────────
    #    🔴🔴 この回は**便名と時刻が話の背骨**なので、崩れた数は全部固定する側に倒した。
    #    ⚠️ 「曖昧なら固定する」＝Scribe は同じ音を2回起こすと結果が変わるので、
    #       1回の聞取で「合っている」と決めない（feedback-verify-your-own-instrument）。
    "2001年": "にせんいちねん",      # 🔴🔴 **動画の1行目**が『2010年』になっていた（→100%）
    "77便と": "ナナジュウナナ便と",   # 🔴🔴 『二十七便』『二七便』→ 七十七便
    "175便は": "ヒャクナナジュウゴ便は",   # 🔴🔴 『七五便』→ 175便
    "93便が": "キュウジュウサン便が",      # 🔴🔴 『二三便』→ 九十三便
    "93便は": "キュウジュウサン便は",      # 🔴🔴 『六十三便』→ 九十三便
    "9時3分": "クジ3分",             # 🔴🔴 『四時三分十一秒』→ 九時三分十一秒（時刻が変わっていた）
    "43,150リットル": "よんまんさんぜんひゃくごじゅうリットル",  # 🔴🔴 『四十三**点**百五十』＝桁が壊れていた（§6 の宿題）
    "11,400ガロン": "いちまんせんよんひゃくガロン",              # 🔴🔴 『十一**点**四百』
    "26,000フィート": "にまんろくせんフィート",                  # 🔴🔴 『二十六千』＝万の位が無い
    "10,700メートル": "いちまんななひゃくメートル",              # 🔴🔴 『十千七百』＝万の位が無い
    "F-15": "エフじゅうご",          # 🔴🔴 『F3』→ F15（戦闘機の型式）
    "ボーイング757": "ボーイングななごなな",   # 🔴🔴 『707』→ 757（機種・100%）
    "175便と": "ヒャクナナジュウゴ便と",       # 🔴🔴 『三七五便』→ 百七十五便。「175便**は**」を直したあとに残っていた
    # ── 3周目に残った文頭の崩れ（2回の全文起こしで**2回とも**崩れたものだけ）──────────
    "軍が": "ぐんが",                # 🔴 『ズンが』→ 軍が（100%）
    "出すには": "だすには",          # 🔴 『バスには』→ 出すには（100%）
    "煙が上がって": "煙が あがって", # 🔴 『煙がなっている』＝**あが が落ちていた** → 煙が上がっている
    "頼んで行って": "頼んで おこなって",  # 🔴 『やっていた』『おっていた』と2回別々に崩れた → 頼んで行っていた
    "燃料の減った": "燃料の へった",      # 🔴 『欠いた』『蹴った』＝3回の聞取で3通り＝読みが安定していない → 減った（100%）
}
# 🔴 **入れないと決めた語**（A/B と2回目の聞取で「音は正しい」と分かったもの。次の回でまた疑わないために残す）:
#   交信（2行）… 聞取「更新」＝どちらも コウシン。Scribe の字の当て違い
#   軍が（pr04-1）… el_yomi.tsv では「ズンが」だったが、**A/B で引き直したら「軍が」100%**。
#                   ＝ Scribe の揺れ（同じ音を2回起こすと結果が変わる＝reference-elevenlabs-tts）。要耳へ
#   1機目（c321-1）… 聞取「一期目」＝ イッキメ で読みは正しい。かなにしても聞取は変わらなかった
#   便（91行）／4便・3機・1機 … 聞取「ヨンビン」「四基」＝読みは正しい
#   あいだ・じゅう・跡・あと … janome が カン／ナカ／ノチ と読んだだけ（Scribe の字は台本どおり）
#   録られて（c803-2）… 聞取「撮られて」＝ トラレテ で同じ
#   F-16（4行）・ボーイング767（1行）… **崩れていない**。F-15 と揃えたくなるが、
#                   A/B で F-16 をかなにすると巻き添えが出た（ラングレー→ダングレー）。
#                   規則は「正しく読めている語をかな化しない」＝入れない
#   首都まで（c622-2）・軍人が（c421-3）・93便は（c805-2）… 1回目の聞取は『千歳／ヒュンジン／六十三便』だったが、
#                   **A/B で引き直したら台本どおり**。Scribe の揺れ。⚠️ 93便は だけは数なので固定する側に倒して入れた
#   一次レーダー（c210-1）… 聞取「一時レーダー」＝ イチジ で同じ読み。字幕が出るので直さない
#   返すのが（c209-2）… 1回目は『返すのね』だったが、A/B の前後とも『返すのが』＝Scribe の揺れ
#   そこから（c712-2）… 『あそこから』。空白で切ると**巻き添えで「向き直って→抜き直って」**になったので入れない → 取り直しへ
#   刃渡り（c113-1）… 同じ「はわたり」を送っていて c904-1 は直り c113-1 だけ『幅たり』＝**テイク差** → 取り直しへ
