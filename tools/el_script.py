...
# ⭐ 2026-09-08 ⑤a 検査で確定した 29件。**すべて A/B（el_ab_yomi）で実測してから入れている**。
#    判定の物差し＝`el_ab_verdict.py`（一致率では決めない。狙った音が後の聞取にだけ現れたか／
#    key の字が後にだけ現れたか／台本の数が後にだけそろったか）。記録＝audio/el_qa/keybridge_ab_verdict.tsv
EL_YOMI = {
    # ── 読みそのものが違っていた語（前の聞取 → 後の聞取）────────────────────
    "水先人": "みずさきにん",        # 推薦人（すいせんにん）→ **水先人**。15行に当たる。
    #  ⚠️ 最初は「みずさき人」にしたが、**人 を ジン と読んだ**（聞取「水崎仁」）。全部かなで一発（el_probe_words で実測）
    "点検員": "てんけん員",          # 弁慶院・宣誓員 → 点検員。15行
    "右舷": "うげん",                # 尾栓・眉山・未然 → 右舷
    "左舷": "さげん",                # 三年 → 左舷
    "端子台": "たんし台",            # 唐時代（とうじだい）→ 端次第（たんしだい＝音は正しくなった）
    "斜張橋": "しゃちょうきょう",    # 七張橋 → **斜張橋**。⚠️「しゃちょう橋」だと 橋 が はし／きょう で揺れた（聞取「社長橋」）
    "引張部材": "ひっぱり部材",      # 引き帳ボロ材 → 引っ張り部材
    "実効的": "じっこう的",          # 一方的 → 実効的
    "積荷": "積み荷",                # 住処 → 積み荷。⚠️ かなもカナも駄目・送り仮名だけ通る（4本目の実測と同型）
    "甲板員": "かんぱん員",          # カンパニー → 甲板員
    "甲板長": "かんぱんちょう",      # 看板町 → **看板長**（かんぱんちょう）。⚠️「かんぱん長」だと **長 を ナガ** と読んだ（聞取「カンパンナガ」）
    "橋台": "きょうだい",            # 貸し台 → 橋台
    "外航船": "がいこう船",          # マイコウ線・内航船（意味が反転）→ 外交船（がいこうせん＝音は正しい）
    "二等航海士": "にとう航海士",    # 尾藤航海士 → 二等航海士（⚠️ 1回目は崩れていない行で試して空振りした）
    # ── アクセント句を半角空白で切る型（かな単独では効かなかったもの）──────────
    "の指針の": "の ししんの",       # 地震 → 指針。⚠️「ししん」単独では『試し』になり効かなかった
    "の指針より": "の ししんより",   # 🔴 上のキーが「1991年の指針**より**前に」に当たらず、c822-1 が『地震』のまま残っていた（書き損じ）
    "橋を抜ける": "はしを抜ける",    # 文頭『橋』→聞取『足』（あし と読んでいた）→ 橋
    "材料研究室": "ざいりょう研究室",  # 文頭『材料』→聞取『微量（びりょう）』→ 材料
    "針路": "しんろ",               # 🔴 聞取『真西140度』『針状142度』＝**進む向きの数値**が別語になる。5行すべてに当てる
    "通れるように": "とおれるように",  # 🔴 文頭。聞取『壊れるように』＝**意味が反転**（開通の話）
    "速力7.5": "そくりょく7.5",     # 文頭。聞取『スク力』→ 速力（100%）
    "崩落から": "ほうらくから",      # 文頭。聞取『風俗から』→ 崩落（100%）。「崩落の前」とは別のキー
    "主航路の上": "しゅこうろの上",  # 文頭。聞取『主砲楼（ほうろう）』→ 主航路。2行とも当たる
    "引外し装置": "ひきはずし装置",  # 聞取『引きずし装置』→ 引き外し装置
    "ワゴ社の3口": "わごしゃの3口",  # 聞取『輪ゴム車』→ 和語社（わごしゃ＝音は正しい）。行中の「ワゴ社」は触らない
    "州交通局は、この": "しゅうこうつうきょくは、この",  # 🔴 文頭。聞取『民衆交通局』→ 週交通局（しゅうこうつうきょく＝音は正しい）。
    #  ⚠️「しゅう交通局」（後ろに漢字）だと『西州交通局』になった＝ここも全部かなが要る
    "低圧が戻る": "テイアツが戻る",   # 🔴 文頭。聞取『血圧』→ **低圧**。ひらがな「ていあつ」では『手厚』になったのでカナ
    "防衝工は壊れた": "ぼうしょう工は壊れた",  # 🔴 文頭。聞取『防潮溝（ぼうちょう）』→ 某商工（ぼうしょうこう＝音は正しい）。
    #  ⚠️ 全部かな「ぼうしょうこう」だと『王昭君』になった＝**かなにしすぎても崩れる**。行中の防衝工は触らない
    "年間の隻数": "ねんかんの せきすう",  # 円環の積数 → 年間の積雪（年間は直った。隻数は要耳）
    "の側": "の がわ",               # そば・ガーガー・護摩 → 側。当たる8行すべて「〜のがわ」で正しい
    # ── 文頭の頭欠け（かなで直る型。5本目 SL-1 の建屋→たてや と同じ）──────────
    "崩落の前": "ほうらくの前",      # 風邪薬の前 → 崩落の前
    "力の道筋": "ちからの道筋",      # からの道筋 → 力の道筋
    "覆ったぶん": "おおったぶん",    # ここをえった分 → 覆った分
    "柱に残った": "はしらに残った",  # まひらに残った → 柱に残った
    "錨のブレーキ": "いかりのブレーキ",  # ミカのブレーキ → 怒りのブレーキ（いかり＝音は正しい）
    "岸壁に戻して": "がんぺきに戻して",  # 岩壁に戻して → 岸壁に戻して（文頭）
    # ── 数はカナ／かなで固定する（reference-elevenlabs-tts）────────────────
    #    ⚠️ 259 と 2769 と 4679 は「同じ音を2回文字起こしすると結果が変わった」＝どちらとも言えない。
    #       数の誤りは意味が変わるので、**曖昧なら固定する**側に倒した。
    "35人": "サンジュウゴにん",              # 75人 → 35人（🔴 別の事故の死者数。意味が変わる）
    "6600ボルト": "ろくせんろっぴゃくボルト",  # 六百ボルト → 六千六百ボルト（🔴 桁が1つ落ちていた）
    "2769メートル": "にせんななひゃくろくじゅうきゅうメートル",  # 二七六九／二千二百六十九 → 二千七百六十九
    "34121台": "さんまんよんせんひゃくにじゅういち台",          # 三四八一二一 → 三万四千百二十一
    "4679個": "よんせんろっぴゃくななじゅうきゅう個",            # 四四曲六七九 → 四千六百七十九
    "259ページ": "にひゃくごじゅうきゅうページ",                # 百五十九／九か十週九 → 二百五十九
    "0時05分": "れいじ ゴふん",     # 🔴 聞取「零時**六**分」＝ご→ろく（時刻が1分ずれる）→ 零時五分
    "29分09秒": "29分 09秒",        # 🔴 聞取「二十九分**二十九**秒」＝09が29に飲まれた（**衝突の瞬間の秒**）。
    #  ⭐ 空白で切るだけで直った（かな化は不要）。他行と同じ「零九秒」に戻る
}
# 🔴 **入れないと決めた語**（A/B と秒数で「音は正しい」と分かったもの。次の回でまた疑わないために残す）:
#   径間（17行）… 前「三計観」も後「三径間」も読みは ケイカン。Scribe の字の当て違い
#   操舵ポンプ  … 前「ソーダポンプ」＝そうだ。かなにすると「そうだ、ポンプ」と間が入って**悪化**した
#   舶用軽油    … 聞取「柏葉桂油」＝はくようけいゆ（同じ読み）
#   防衝工      … 聞取「防衝構」＝ぼうしょうこう（同じ読み）
#   主機関      … 聞取「手記感」＝しゅきかん（同じ読み）。かなにすると「機種機関」で悪化
#   非冗長・岸壁（行中）… 聞取が台本どおり
#   補修班      … 聞取「保守派」だが**実測 0.681秒 ≒5.7モーラ**＝ほしゅうはん（5）で ほしゅはん（4）でない
#   航路        … 聞取「坑道」だが**実測 0.280秒 ≒2.6モーラ**＝こうろ（3）で こうどう（4）でない
#   寄与要因    … 「きよ要因」「きよ よういん」「きよよういん は」の3通りとも駄目（清陽院／火要因）＝**要耳へ回す**
#   船首には    … 「せんしゅには」で悪化（甲板長まで巻き添えで崩れた）＝取り直しへ
#   ワゴ社      … 「わご社」でも聞取は『我が社』のまま＝わごしゃ と読めている（Scribe の字の当て違い）
#   外航船（文頭）… 「がいこうせん」だと **が が か に落ちた**（聞取「海溝線」）。行中の「がいこう船」は正しい＝文頭は取り直しへ
EL_YOMI_OPEN_RIGHT = frozenset()
# 🔴「引張部材」は台本では必ず「非冗長鋼**引張部材**」＝直前が漢字なので、既定の境界規則
#    （漢字で始まるキーは直前が漢字なら当てない＝複合語の内部に当てないための守り）に弾かれる。
#    ここだけ左の境界を開ける。⚠️ 開けたぶん「〜引張部材」の全部に当たるが、この台本では2行とも狙いどおり。
#    （import 時の門番が「当たらないキー」で落ちて気づいた＝黙って素通りしていない）
EL_YOMI_OPEN_LEFT = frozenset({"引張部材", "引外し装置"})   # どちらも直前が漢字（非冗長鋼〜／不足電圧〜）
