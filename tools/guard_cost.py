...
■ 「絶対に課金しない」の担保（2026-08-02 カズヤくん指示）
  いちばん効いているのは**この道具ではなく、ワークスペースの予算設定**：
      ...
      予算 $10  <  クレジット $30/月      （2026-09-21 に $5→$10）

  予算は「**使用量**がその額に達したらアプリを停止」する仕組みで、
  クレジットは使用量を $30 まで吸収する。$10 で止まる時点で必ず $30 の内側なので、
...
BUDGET_USD = 10.00       # ワークスペースに設定した支出上限（ダッシュボード側の実値）
#   2026-09-21 カズヤくんが $5→$10 に変更。2026-09-24 に画面で確認
#   （Usage & billing → Usage this billing period → Usage limit「$10・Set by you」）。
#   ⚠️ CLI は上限を表示しない＝ダッシュボードで変えたら、ここも手で合わせる
