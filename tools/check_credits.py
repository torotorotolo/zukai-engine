...
#    🔴🔴 2026-09-23（12本目 キャッスル・ブラボー ⑤b-2）：**12本目へ切り替えた。**11本目の値は
#       `EP_PREFIX="ep11/"` ／ 見出しの最後の列が「出どころ」。
#       12本目は**所蔵が3つ**（NARA の naId・Commons の題名・朝日グラフ 1954）なので、
#       最後の列を「**所蔵と識別子**」にして 11本目（出どころ）と分けた。
#       ⚠️ `ref/CREDITS.md` に §キャッスル・ブラボー の表を足すまで `load_table()` は**止まる**（正しい）。
#    🔴 2026-09-24（13本目 トルコ航空981便 ⑤b-1）：`ep13/` へ（§0b の7か所目）。
#    🔴🔴 2026-09-24（13本目 ⑤b-2）：⑤b-1 は「見出しの形は12本目のまま・13本目の表を足すまで止まる（正しい）」と
#       書いたが、**止まっていなかった**。見出しが12本目と同じ文字列なので `lines.index(HEADER)` が12本目の表
