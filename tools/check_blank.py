...
# 12本目 ⑤c'（09-23）：scan に trim を通したら鳴った2件。どちらも ⑤c' で絵を1画素も変えていない
#    （r02 のシート全数＝⑤c で空白の指摘なし・c513 は走査の線の候補として目で見ている）＝鳴ったのは測り方が本番に揃ったから。
#    ⚠️ INK_MIN（4.0%）は trim を通さない窓で決めた値。通したら76カットでインク率が上がった15・下がった12
#       （c519 6.8→2.9%・c513 5.8→3.9%・c719 3.7→7.3%）。次の題材で下限を本番の全数から取り直す。
ON_PURPOSE = {
    "c513": "報告書の第59図（1時間後の放射線・pg219）＝島と等値線だけの線画で疎。インク 3.9%・空白帯 14%（頁まるごとの窓では 5.8%）",
    "c519": "報告書の第60図（3時間後の降灰の再現・pg221）＝海の地図に等値線の線画。東へ流す窓の尻でインク 2.9%・空白帯 7%"
            "（頁まるごとの窓では 6.8%）",
}
