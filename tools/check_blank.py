...
#    ✅ 2026-09-25（13本目 ⑤c'）に13本目の本番45カット（trim を通した窓・k=0/1）で取り直した＝最小 4.5%
#       （c314 pg88・c904 pg4101）・次が 5.1%（c604）。事故の最大 3.8% との谷に 4.0% が入ったまま＝**据え置き**
...
def scan(spec_map, photo_of, box_of, skip):
    ...
    for cid in sorted(spec_map):
        ...
        spec = spec_map[cid]
        arr = load_arr(name)
        # 🔴 2026-09-23（12本目 ⑤c'）：**trim を通す**（本番は `load_photo` で先に切り落としてから fit に渡す）。
        #    ここは頁まるごとの上で窓を組んでいたので、寄りを下に固定した c719（bias 1.0）の窓が
        #    頁の下の余白（「Note:」の行）へ落ち、インク率 3.7% と**画面に出ない所を測って**止めた
        #    （画面に出る窓では 7.9〜8.2%）。check_slide は 6本目に同じ穴を直していた
        #    （[[feedback-gates-must-share-the-production-geometry]]）。切るのは配列の view＝キャッシュは原寸のまま。
        tr = CS.trim_of(spec)
        if tr:
            H, W = arr.shape[:2]
            arr = arr[round(tr[1] * H):round(tr[3] * H), round(tr[0] * W):round(tr[2] * W)]
...
# 12本目 ⑤c'（09-23）：scan に trim を通したら鳴った2件（c513・c519）を載せていた＝git の `3832147`。
#    ⚠️ INK_MIN（4.0%）は trim を通さない窓で決めた値。通したら76カットでインク率が上がった15・下がった12
#       （c519 6.8→2.9%・c513 5.8→3.9%・c719 3.7→7.3%）。次の題材で下限を本番の全数から取り直す。
# 🔴 2026-09-24（13本目 ⑤b-1）：**空にした**（§0b の一覧に無かった13か所目＝カットIDの直書き）。
#    12本目の c513・c519 は13本目にも同じIDがある＝残すと13本目の c513・c519 が黙って免除される。
#    🔴 INK_MIN の取り直しは、13本目の頁を当てたあと（⑤b の終わり・qa_all の前）に本番の全数で。
ON_PURPOSE = {}
