...
# 🔴 実測（2026-08-01）。**この曲は 80〜400Hz にエネルギーの 50.9% がある**
#    （自作ドローンは 9.8%）。そこは語尾の「ン」が鳴る帯そのものなので、
#    **同じ音量ならドローンの5倍マスクしやすい。** 音量は耳でなく測って決めた。
#      実効音量  消える  弱い  中央値
#        -40dB     0行    0行  32.8dB
#      **-38dB     0行    1行  30.8dB**  ← 採用（カズヤくん指示「控えめに」）
#        -36dB     0行    2行  28.8dB    ← 語尾が埋もれない上限
#        -34dB     1行    2行  26.8dB    🔴 ここから語尾が消え始める
#    ⚠️ 曲を替えたら**必ずこの表を取り直す**（`python tools/check_mask.py`）。
#      帯域の分布が違えば、同じ音量でも結果はまったく変わる。
#    削ったあとの実測（全417行の語尾で判定）
#      -32dB 消える0/弱い1   **-30dB 0/2（採用・上限）**   -28dB 1/2 🔴
#    削る前は -36dB が上限だったので、**カーブで 6dB ぶんの余裕ができた**。
#    ⚠️ 話しているあいだはサイドチェインでさらに -7dB。無音の間だけ元の高さに戻る。
BGM_RMS = -30.0          # BGMだけのときの実効音量（dBFS RMS）
...
#    この曲の帯域は 80-400Hz が 50.9%／400-2000Hz が 48.5%／2kHz以上はほぼ0%。
#    語尾の「ン」が居るのは 80-400Hz だけなので、**そこを削れば残り半分は自由に上げられる**。
#    → 音楽としての存在感（中音）は残したまま、マスクだけ減らせる。
BGM_CARVE = (90, 420, -12.0)     # (下端Hz, 上端Hz, 何dB下げるか)
