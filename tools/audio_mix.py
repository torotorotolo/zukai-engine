...
# 🔴 2026-09-21（10本目）: 曲を **「疑惑の霧」／Keido Honda（OpenTracks・旧 DOVA-SYNDROME）** に替えた
#    （カズヤくん指定 https://opentracks.com/bgm/detail/4650 ）。
#    ⚠️ **クレジット表記は要らない**（ライセンス原文「著作権表示・提供等の表示は不要です。」）。
#      必須でないクレジットは書かない決まりなので、**概要欄にこの行は入れない**
#      （[[feedback-no-optional-credits]]）。ここに残すのは「何で焼いたか」の記録として。
BGM_CREDIT = "BGM：疑惑の霧／Keido Honda（OpenTracks）"   # ⚠️ 概要欄には入れない（表示は不要）
...
#    🔴 2026-09-15（8本目 ⑥の試写）カズヤくん「**BGMが大きい。小さくしてください**」。
#      -27.0 → **-30.0**（さらに -3dB）。段の取り方は 09-07 と同じ理由
#      （3dB＝はっきり小さくなったと分かる最小の段。-1〜-2 は聴き分けられない）。
#      ⚠️ このときの機械の実測は **全体 -15.0 LUFS ／ 声 -14.8 ／ BGMだけ -33.4〜-31.2
#         ＝差 18.5 LU** で、7本目（18.2 LU）より**既に開いていた**。それでも耳は「大きい」。
#         ＝ 09-07 の申し送り「決着は耳。機械の実効値だけで動かさない」がそのまま出た。
#      ⚠️ 下げる方向なので語尾のマスキングは悪化しない（余裕は増える側）。削り -18.0 は据え置き。
# 🔴🔴 2026-09-21（10本目）カズヤくん指示「**BGMはかなり小さめで設定してください**」。
#    段は 3dB（はっきり小さくなったと分かる最小の段）なので、**2段ぶん下げて -30.0 → -36.0**。
#    「少し小さく」なら1段（-33.0）、「かなり」なので2段を採った。
#    ⚠️ 実測：疑惑の霧（133.5秒・素の RMS -13.3 dBFS）のエネルギーの置き場所は
#       0-90Hz **28.9%** ／ 90-420Hz **51.0%** ／ 420-2000Hz 19.3% ／ 2kHz以上 0.8%。
#       ＝前の曲（陰鬱な灰色の気配・90-420Hz が 52.3%）と**同じ型**で、
#         語尾の「ン」が鳴る帯に半分がある。**だから下の削り -18.0 はそのまま効く**
#         （[[feedback-bgm-carve-before-raising]]＝上げる前に削る。下げる回も削りは外さない）。
#    ⚠️ この回の語りはほぼ途切れない（語りの無い区間は 2248秒のうち 174秒＝7.7%）。
#       -36.0 にサイドチェイン -7dB が乗ると、話している間は実質 -43dB＝**ほぼ聞こえない**。
#       聞こえるのは章の変わり目などの空きだけになる。**決着は試写の耳**（機械の実効値で動かさない）。
# 🔴 2026-09-21 試写：カズヤくん「**BGMをもう少し大きく**」。-36.0 は下げすぎだった（実測＝声との差
#    **27.2 LU**。09-15 に「まだ大きい」と言われた版が 18.5 LU）。**1段（3dB）だけ戻して -33.0**。
#    ＝差は約 24 LU の見込み。「かなり小さめ」の指示は生きているので、-30.0 までは戻さない。
BGM_RMS = -33.0          # BGMだけのときの実効音量（dBFS RMS）★2026-09-21 -36.0 から3dB戻した
...
# 🔴🔴 2026-09-20（10本目 ⑤b-1）**1本目タイタン号のカットIDのまま残っていたので空にした。**
#    → [[project-jiko-rules-index]] §0b の9か所目。門番は無い（鳴らずに黙って鳴る）。
#    ⚠️ 10本目にも **pr01・c614〜c617 は実在する**＝そのままだと
#       「被害者は1,439人である」（c614）の下に心拍が鳴っていた。
#       これは上の「**人が亡くなる場面には入れない**」に自分で違反する。
#    🔴 心拍・衝撃音を使う回は、**その回のカットIDで決め直してから**ここに書く。
#       BGM を鳴らすかどうかは回ごとに `--bgm music|none` で明示する（既定値なし）。
HEART_CUTS = set()
