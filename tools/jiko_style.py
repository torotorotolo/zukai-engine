...
AMBER = "#e8b33c"         # 数値
OK = "#5fbf8f"
# 🔴 2026-08-01（r13 試写「目盛りの文字が背景と同化して読めない」）。
#    目盛りと注記は LINE_DIM(#41606f) で書いていたが、**地との比を測ったら 2.64:1**
#    しか無かった。WCAG は小さい文字で 4.5:1 が下限なので、読めないのは当然だった。
#      GRID 1.36 ／ LINE_DIM 2.64 ／ ALERT 4.55 ／ TICK 6.39 ／ LINE 8.21 ／
#      AMBER 9.26 ／ OK 7.89 ／ INK_W 15.67  （すべて BG #0f1922 に対して実測）
#    → **文字に LINE_DIM を使わない。** 目盛り・注記・軸名は TICK にする。
#      LINE_DIM は「線を沈める」用（罫・目盛りの短い線・破線）としては残す。
TICK = "#7fa0b0"          # 目盛り・注記の文字（地との比 6.39:1）
# 🔴 2026-08-02（カズヤくん指摘「色がまだ増えていない。TICK を足しただけ」）。
#    前の巡は TICK（LINE の暗い版・ΔE 8.2）しか足していないので、
#    **画面に出る色の数は1つも増えていなかった**。
#    → 意味を持たせて2色足す。`tools/check_color.py` で実測して選んだ。
#      条件：地との比 4.5 以上（文字に使える）／既存の生きた色から ΔE 30 以上／
#            彩度は既存の帯（LINE 0.29〜OK 0.50）に収める（派手な色は様式を壊す）。
#      INST #ac93d4  比 6.66  S=0.31  最も近い既存色 TICK から ΔE 34.8
#      DOC  #c2a478  比 7.51  S=0.38  最も近い既存色 AMBER から ΔE 38.5
#    ⚠️ 色を足すだけでは足りない。**カット側で使い分けて初めて増える**（下の表）。
INST = "#ac93d4"          # 第三者機関・制度（船級協会・沿岸警備隊・OSHA・規格・登録）
INST_DIM = "#5c4d75"      # その沈めた版（比 1.95。線・地の面に使う。文字には使わない）
DOC = "#c2a478"           # 書類・記録・データ（報告書・書簡・ログ・引用の出どころ）
DOC_DIM = "#6e5c42"       # その沈めた版（比 2.09）
# ── 色の使い分け（226カットで守る）─────────────────────────
#   INK_W  主図形（船体・断面）      LINE   技術線・物
#   AMBER  数値                      ALERT  破壊・欠陥・会社が選んだ危ないほう
