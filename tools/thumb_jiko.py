...
# ── タイタン号（本番1本目）の3案 ───────────────────────────
# 写真はすべて NTSB/MIR-25-36 の図版＝**NTSB が撮影した米連邦機関の著作物＝PD**。
# ⚠️ 報告書に転載された OceanGate 提供の図（図1・2・19・20）は権利が残るので使わない。
CR = "写真：NTSB／MIR-25-36（パブリックドメイン）"
PH = "titan_hull_edge.jpg"          # 図14下：中央破断面。層が刃のように裂けている
...
def titan():
    ...
    """
    uri = photo(PH, cy=0.52, contrast=1.24, color=1.06, bright=0.92)
    bake("titan_A", type_a(
        uri, "ダイブ80", "で壊れていた",
        "5名死亡・そのあと3回、同じ船でタイタニックへ",
        "タイタン号 爆縮", "炭素繊維の層間剥離", "潜水", "2023・北大西洋", CR))
    bake("titan_B", type_b(
        uri, "壊れる音", "を聞いていた",
        "乗員5名全員死亡・深さ3,363mで爆縮",
        "タイタン号 爆縮", "炭素繊維の層間剥離", "潜水", "2023・北大西洋", CR))
    bake("titan_C", type_c(
        uri, "3,363", "メートルの深さで爆縮", "5名が死亡した水深",
        "壊れたあと3回潜った", "潜水艇タイタン号 5名死亡",
        "炭素繊維の層間剥離", "潜水", "2023・北大西洋", CR))
