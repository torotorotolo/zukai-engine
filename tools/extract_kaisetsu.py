...
CROP = [
    ...
    # 🔴 2026-08-04：表の**左の罫線が半分に切れて**いた（罫は x=65.28、x0 は 68）。
    ("kh001.png",  7, (62.5, 55, 545, 232),
     "解説書 表1", "急減圧時の機内現象についての実際の体験と事故前の認識（推定）"),
    ("kh002.png",  7, (68, 448, 545, 690),
     "解説書 表2", "急減圧後、客室内の高度が10,000ftに上昇するまでの時間（概算）"),
    # 🔴 2026-08-04（r01 の拡大目視）：左端に**図の枠の外の青い縦線**が入っていた。
    #    実測：x=70.92（RGB 0,0.439,0.753・幅0.75pt・y257.0〜296.8）で、
    #    図の枠は x=74.10 から。JTSB が引いたまま残った線で、図の一部ではない。
    #    → x0 を 68 → 72.5（青の右端 71.28 と枠の左端 74.10 のあいだ）。
    ("kz001.png",  8, (72.5, 112, 545, 494),
     "解説書 図1", "急減圧後、客室（与圧室）内の圧力の推移"),
    # 🔴 同上：**タイトル「図2 川の例」が上端で半分に切れて**いた（実測 y268.6〜281.2）。
    #    右も、図の外枠 x=565.7 と青い矢印 x=548.7 を x1=545 で切っていた。
    #    → y0 272 → 264.5（1つ上の本文は y262.2 で終わる）／x1 545 → 570。
    ("kz002.png",  9, (68, 264.5, 570, 472),
     "解説書 図2", "川の例（流れの断面積と速さ）"),
    ("kz009.png", 14, (95, 212, 530, 542),
     "解説書 図9", "温度回復のシミュレーション"),
    # 🔴 2026-08-04：図11 の写真（x=96.60 から）を x0=105 で 8pt 切っていた。
    ("k_keiki.png", 23, (93, 70, 548, 245),
     "解説書 図11・図12", "当時の計器（距離表示と方位表示）"),
    # 🔴 2026-08-04（r01 の拡大目視）：表の下に**次の枠（機体写真の枠）の上辺**が
    #    5px 入っていた。実測：表の下辺 y=658.0、次の枠 y=663.4。
    #    → y1 664 → 661。
    ("kh003.png", 23, (65, 326, 548, 661),
