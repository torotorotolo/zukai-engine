...
def panel(blocks, lead="", note="", cols=3):
    ...
    if n <= 3 and all(len(str(b.get("t", ""))) <= 46 for b in blocks):
        ...
        for i, b in enumerate(blocks):
            y = top + i * h
            c = b.get("c", J.LINE)
            # 🔴 同（K-09）：段の下に**書類の罫**を1本引く。空きを「右に大きな穴」から
            ...
            # 🔴 2026-09-07（5本目 SL-1 ⑤c'・K-09）：**上限 96 / h*0.52 が低すぎた。**
            #    この型の 8カットで空き矩形が 27.8〜41.7%（本編でいちばん空いている）。
            #    中身は 1〜12字（「電気」「鋼」「板」）なので、**器を広げても埋まらない**
...
def mapfig(points, note="", link=None, scale=None, lead="", coast=None, turn=None):
    ...
    for p in points:
        ...
        # 🔴🔴 2026-09-10（6本目キー橋 ⑤c 検品【見る】4周目・台帳 §X-1-4）：
        #    札の side を**点の x だけで決めていた**ので、`c105` の「シーガート」
        #    （x=0.18 → 札は右）が、そのまま右へ伸びる `link` の破線の上に載った。
        #    実測：破線 M417.3 421.6 → L790.4 433.1／札の字面 x 451〜621・y 409〜424
        #    ＝ **札の下側を線が通る**（取り消し線に見える）。
        #    ⚠️ `check_layout` の「図形が文字を横切る」は、**文字があとの層に描かれると
...
def people(nodes, edges=None, note="", lead="", src=""):
    ...
    for e in (edges or []):
        ...
        if e.get("t"):
            ...
            #    片側 60px は要る。
            # 🔴 2026-09-10（6本目キー橋 ⑤c 検品【見る】4周目・台帳 §X-1-5）：
            #    **札が別の節の箱に載っていた**（`c410` の2つ目の「問い」＝
            #    札 x630〜732・y405〜463 が「二等航海士」の箱 x680〜1171・y266〜456 と
            #    **52×51px** 重なる。箱の左下の角をまたいでいた）。
            #    真因は、軸の長さだけを見て「札を軸の真ん中に置ける」と決めていたこと。
            #    斜めの辺は、**途中で他の節の箱の上を通る**ことがある。
            #    → 軸に載せる前に、**他の節の箱と重ならないか**を測る。重なるなら
            #      下の枝（矢印は通しで引き、札は直角へ逃がす）へ回す。
            #    ⚠️ 逃がす向きは、下の枝がすでに持っている規則をそのまま使う。
            cents = [C(i) for i in range(len(nodes))]

            def clear(px_, py_, skip=()):
                """その位置に札を置いたとき、どの節の箱とも重ならないか。"""
                return not any(
                    abs(px_ - cx) < (bw + tw) / 2 and abs(py_ - cy) < (bh + th) / 2
                    for i, (cx, cy) in enumerate(cents) if i not in skip)

            if ln - half * 2 >= 120 and clear(mx, my, skip=(e["a"], e["b"])):
                ...
                if abs(uy) > abs(ux):          # 縦向きの矢印は横へ逃がす
                    ...
                    nx, ny = -nx, -ny
                # 🔴 2026-09-10（6本目キー橋 ⑤c 検品【見る】4周目・台帳 §Y-1-6）：
                #    **札が矢印から離れて空白に浮いていた**（`pr11`「0128:21　封鎖」＝
                #    矢印 x≈1600 に対して札 x 1070〜1350）。
                #    真因はここ。縦向きのとき「節の箱の幅の半分」（`bw/2`≒245px）だけ
                #    横へ逃がしていたが、**節は矢印の上と下にいる**ので、
                #    札は箱を避ける必要がない。避けるのは**線**だけ。
                #    → まず線を避けるぶんだけ寄せ、**箱と縦に重なるときだけ**箱ぶんへ広げる。
                #    ⚠️ 「箱と縦に重なる」は実測で判定する（`bh` と札の高さ `th` で）。
                #    ⚠️ 横向きの矢印（`elif`）はここまでどおり。触らない。
                if abs(uy) > abs(ux):
                    near = any(abs(my - cy) < (bh + th) / 2 for _cx, cy in (a, b))
                    if not near:
                        d0 = tw / 2 + 16
                # 🔴🔴 2026-09-10（⑤c 検品【見る】4周目・台帳 §X-1-5）：
                #    **逃がした先が、別の節の箱の上だった**（`c410` の2つ目の「問い」＝
                #    札 x630〜732・y405〜463 が「二等航海士」の箱 x680〜1171・y266〜456 と
                #    **52×51px** 重なる。箱の左下の角をまたいでいた）。
                #    ⚠️ 最初、**逃がす前の位置**で重なりを見て直したつもりになった。
                #      判定は必ず**置いたあとの座標**で取る（[[feedback-verify-your-own-instrument]]）。
                #    → 反対側・さらに遠く、の順に試して、**箱に載らない最初の場所**を採る。
                #      どこも空いていなければ元の向きのまま（絵が壊れるより載るほうがまし）。
                for sx, sy, dd in [(nx, ny, d0), (-nx, -ny, d0),
                                   (nx, ny, d0 + bh / 2), (-nx, -ny, d0 + bh / 2)]:
                    if clear(mx + sx * dd, my + sy * dd):
                        nx, ny, d0 = sx, sy, dd
                        break
...
def truss(mode="flow", lead="", note="", marks=None, hot_panel=None):
    ...
    # 🔴 2026-09-10（6本目キー橋 ⑤c' 4巡目）：**上の直しは左端でしか成り立っていなかった。**
    #    `dy(x)` は左端で 96・右端で 0 なので、下弦材は右へ行くほど**上がってくる**。
    #    札の字面は yb+55〜yb+83 なので、`dy` が 55〜83 になる区間
    #    （＝枠の左から 13.5%〜42.7%）で**下弦材が札を貫く**。
    #    実測（`c613`・新設の `qa_out/kb_y_cross.py`）：白い下弦材が
    #    「車が通るのは、骨組みの中」を横切っていて、原寸でも取り消し線に見えた。
    #    → 札を**右へ寄せる**。右端では `dy` が 0〜24 なので、下弦材は札より上を通る。
    #    ⚠️ `mode="member"`（`fall=0`）でも下弦材は yb なので、右寄せで問題ない。
    g.append(txt(x1 - 60, yb + 76, "車が通るのは、骨組みの中", 28, J.TICK,
                 "Noto", "end"))
    # 橋脚（"support" では左が無い）
