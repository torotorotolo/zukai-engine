...
USE = {
    ...
    # 🔴 2026-09-14（⑤c'）**bias を 0.52 → 0.62 に上げた。**⑤c の A-4 で実測したところ、
    #    0.52 だと切り出し窓は素材の y87〜942 で、**アプリのメニュー帯（y2〜92）の下端が
    #    5px ぶん窓に残り**、焼き上がりでパネル上端に幅810pxの明るい帯として出ていた
    #    （⑤c-2 §E-2 で3欄とも実測 7px・明るさ191 対 地80）。
    #    余りは 1024-855＝169px なので 0.62 で上端 104px＝帯の下端 92 を 12px 越える。
    #    下は 104+855＝959 で、素材の下端 1024 までまだ 65px 余る（タスクバーは外のまま）。
    "c511": dict(clip="5-ZID-819-USA-System-Shutdown-fr20-spd-400.avi",
                 start=2.0, until=54.0, zoom=1.20, bias=0.62),
    "c512": dict(clip="5-ZID-819-USA-System-Startup-fr20-spd-1600.avi",
                 start=2.0, until=41.0, zoom=1.20, bias=0.62),
    ...
    "c514": dict(clip="5-ZID-819-USA-System-Shutdown-NS-NF-atlantic-focus-fr20.avi",
                 start=2.0, until=49.0, zoom=1.20, bias=0.62),
    ...
    # 🔴🔴🔴 2026-09-14（⑤c'）**c716 の秒を実測で取り直した。**
    #    ⑤c-2 §B＝31.0 からの10秒は**まるごと終幕カード「End of Recording」**だった
    #    （額の中の平均 42.3・明るい画素 4.0%）。`until=41.0` は札の上を指していた。
    #    30.0〜42.0秒を 0.5秒刻み、20.0〜31.0秒を 1.0秒刻みで当たった実測
    #    （`qa_out/ep7_c716_probe.py`・出力 `qa_out/ep7_c716_probe.txt`）:
    #      20〜24秒  平均 210〜220・ばらつき 7〜25・明るい 93〜100% ＝**ほぼ一色の黄色**
    #      25〜35.5秒 平均 111〜143・ばらつき 32〜45         ＝⭐**中身がある 10.5秒**
    #      35.75〜37.25秒 平均 164〜178・明るい 56〜66%      ＝別の明るい場面
    #      37.5秒〜   平均 0.8〜9.8                          ＝**カード（真っ黒＋白い字）**
    #    ⚠️ 1通目の当たり「s1（12〜30秒）へ振り替える」は**誤り**（s1 は黄色一色）。
    #    ⚠️ ②のショット見本は s1=12〜30 としていたが、**25〜30 には中身がある**。
    #      見本は1コマなので、ショットの切れ目はコマを当たるまで決まらない。
    #    ⚠️ **25.5 から始めると `shots.json` のショット（12〜30秒）をまたぐ**（門番 exit 3）。
    #      私の実測では 25〜35.5 は絵が続いているが、**ショットの帳のほうを書き換えない**
    #      （[[feedback-dont-spell-around-a-gate]]＝門番をかわす向きに直さない）。
    #      → ショット「30〜42秒」の中に収め、**足りない尺は 0.62倍速で埋める**。
    #        30.0 + 8.61×0.62 ＝ 35.34 で、明るい場面の始まり 35.75 の手前で終わる。
    #      レーダーと地図の画面なので、ゆっくり出るほうがむしろ読める。
    "c716": dict(clip="1-AWA-714-Pentagon_more2.mp4", start=30.0, until=35.6,
                 rate=0.62),
