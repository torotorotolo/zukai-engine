...
_CLIPS_JSON = HERE / "ref" / "ep8" / "clips.json"
...
SHOTS = {}
SHOT_FILE = HERE / "ref" / "ep8" / "shots.json"           # 8本目。7本目は ref/ep7/shots.json
...
NOGO = {
    # ══════════════════════════════════════════════════════════
    # 🔴🔴 2026-09-14（8本目 コロンビア号 ⑤b-1）**②の実測から3本ぶん入れた。**
    #   ⚠️ 残り6本（`mct` `mc0201` `fdcomm` `fd16` `fdbrief` `cabin`）は **0件だが、
    #      それは「危険が無い」ではなく「まだ見ていない」**（7本目 ⑤c' で実際に踏んだ）。
    #      → ⑤b-2 でショットのシートを見たときに、見つけたぶんをここへ書く。
    #   ⚠️ 7本目の14本ぶんは**下にそのまま残してある**が、クリップ名が違うので
    #      8本目では1件も当たらない（`CLIPS` に無い名前は `in_nogo()` が飛ばす）。
    #      消さないのは、**「同じ型だから同じ秒を弾く」が誤りだった実例**
    #      （`5-AWA-213-ny_best.mp4` には頭の表題カードが無かった）を残すため。
    # ══════════════════════════════════════════════════════════
    #   `guncam` … 0〜9秒は**投稿者が付けた英語の表紙**
    #              （"rare footage captured on Apache Helicopter"）。②で実測
    #   `sts1`   … 0〜10秒は **NASA の表紙**（連絡先つき）。②で実測
    #   `tank`   … タンクが写るのは **12〜22秒と34〜44秒だけ**。
    #              残り約3分は **2011年に撮られた NASA の解説者**＝
    #              「8年半後に湖から出てきたタンク」の絵ではない。
    #              ⚠️ 時代の食い違いは門番が1件も鳴らない
    #              → [[feedback-fallback-stills-must-match-the-era]]
    "guncam": [(0.0, 9.0, "投稿者が付けた英語の表紙")],
    "sts1": [(0.0, 10.0, "NASA の表紙（連絡先つき）")],
    "tank": [(0.0, 12.0, "2011年の解説者（タンクが写っていない）"),
             (22.0, 34.0, "2011年の解説者（タンクが写っていない）"),
             (44.0, 9999.0, "2011年の解説者（タンクが写っていない）")],
    # ── 以下は7本目（9.11）の記録。8本目のクリップ名とは当たらない ────
    ...
    # 🔴🔴 2026-09-14（7本目 ⑤c'）: **`1-AWA-714-Pentagon_more2.mp4` の登録が0件だった。**
    #   ＝ `c716` が「End of Recording」を10秒まるごと映していても `--check` は緑のまま通った
    #     （⑤c-2 §B）。0件は「危険が無い」ではなく「まだ見ていない」
    #     （[[feedback-jiko-plain-language]] の隣＝[[feedback-rules-need-gates]]）。
    #   実測＝`qa_out/ep7_c716_probe.py`（0.5秒／1.0秒／0.25秒刻み・出力はテキストに残した）。
    #   ⚠️ 12〜18秒は当たっていない。②のショット見本 s1（12〜30秒・ほぼ一色の黄色）に
    #     合わせて広めに取った。**弾きすぎる側に倒す**（使える絵を捨てても嘘は出ない）。
    "1-AWA-714-Pentagon_more2.mp4": [
        (12.0, 24.5, "ほぼ一色の黄色＝中身が無い（20.0〜24.0秒を実測・ばらつき 6.8〜25.3）"),
        (37.3, 9999.0, "End of Recording（37.5秒で平均 0.8＝真っ黒に白い字）")],
    #
...
USE = {
    ...
    # ══════════════════════════════════════════════════════════
    #
    # 🔴 8本目の割り当て（台本 §5-1 で決まっている。**秒は⑤b-2 でコマを見てから書く**）
    #
    #   | clip     | 当てるカット                  | 扱い                                |
    #   |----------|-------------------------------|-------------------------------------|
    #   | `mct`    | `c705` ほか `c7`              | 🔴 唯一の全画面（絵の幅 1918）      |
    #   |          |                               | 画面は **GMT**。−5時間で EST に直す |
    #   | `guncam` | `c725` のみ                   | 🔴 空中分解そのもの。**9秒以降**    |
    #   |          |                               | HUD は **Zulu**。英字は隠すか切る   |
    #   | `mc0201` | `c701` `c713` `c720` `c727` `c622` | 額装（絵の幅 646）。2月1日の管制室 |
    #   | `fdcomm` | `c712`〜`c714`                | 額装（絵の幅 **984**。器は 1280）   |
    #   | `sts1`   | `c101` `c102`                 | 額装（絵の幅 948）。**10秒以降**    |
    #   | `cabin`  | `c411` **だけ**               | 額装（絵の幅 315）。⚠️ `c7` では使わない |
    #   | `tank`   | `ep01`〜`ep05`                | 全画面。⚠️ タンクは 12〜22秒と34〜44秒だけ |
    #   | `fd16`   | `c418`                        | 額装（絵の幅 614）                  |
    #   | `fdbrief`| 使わない                      | 台本 §5-1                           |
    #
    # 🔴 秒を書くときに守ること（守らないと `fetch --check` が止まる）
    #   1. `start` / `until` は `ref/ep8/shots.json`（1秒刻み・**502ショット**）の
    #      **境目をそのまま写す**。目分量で書かない
    #   2. **`until=` は全欄に必ず書く**（`still=True` の欄も。無いと exit 2）
    #   3. (start, until) は**1本のショットに収める**（またぐと exit 3）
    #   4. `rate` は「そのショットの残り ÷ カットの尺」を切り捨てた値。機械で出す
    #
    # ⚠️ **⑤b-1 は秒を書いていない。**この環境に OCR が無く、
    #    「どのショットに何が写っているか」を機械で読めなかったため
    #    （`tesseract` も `pytesseract` も入っていない）。
    #    ショットの長さと motion だけで決めると **推測を実測の口調で書くこと**になる
    #    → [[feedback-dont-state-inferences-as-findings]]。
    #    ⑤b-2 で 640px のシートを焼いてショットを見てから書く
    #    （このchは 640px シートでの全数確認が認められている＝`~/.claude/CLAUDE.md` 鉄則1）。
    #
    # ══════════════════════════════════════════════════════════
    # 🔴 2026-09-15（⑤b-2）**ショットを見てから書いた。**
    #   道具＝`qa_out/ep8_shotsheet.py`（1ショット1コマ・640px・2列×3行。
    #   ⚠️ `-user_agent` は file: では通らず、黙って灰色の器で埋まる。取れた枚数を数える）。
    #   `sts1` 20ショットと `guncam` 7ショットは**全数**見た。`mct` `tank` `mc0201`
    #   `fdcomm` `fd16` `cabin` は当てる秒のまわりを見た。
    #   ⚠️ `start`/`until` は `ref/ep8/shots.json` の境目そのもの。`until` は
    #      **ショットの終わり −1.0秒**（境目は後ろ側の秒に置かれるので、
    #      until＝境目のままだと尻に次のショットが最大1秒混じる）。
    # ══════════════════════════════════════════════════════════
    #
    # ── `sts1`（STS-1 40周年の記録映像。器1280 の中の絵は948）────────
    #   #05 42〜60秒＝上昇（追跡カメラ。43秒と49秒で同じ絵を確かめた）
    #   ⚠️ **#01（10〜22秒）はショットの中で絵が3つに変わる**（11秒は真っ黒＝表紙からの
    #      溶暗／14秒は着装の寄り／20秒は乗員の通路）。1秒刻みで見て捨てた。
    #      → [[feedback-measure-the-source-before-choosing-the-crop]]
    #   ⚠️ **#02（22〜33秒）も30秒から射点の絵に変わる**（30秒・31秒で実測）。
    #      c102 が使えるのは **23.0〜29.5秒**（乗員の通路＋着装）だけ。
    #   ⚠️ **尺は台本の「秒」ではない**（LEAD/TAIL が付くので 1〜2秒長い）。
    #      窓は `scene_jiko.CUTS` の実尺で取ること（c102 は 7.33秒）。
    "c101": dict(clip="sts1", start=42.5, until=59.0),
    "c102": dict(clip="sts1", start=22.4, until=29.9),
    #
    # ── `fd16`（飛行16日目の会見。器615 の中の絵は614）──────────────
    #   #04 27〜171秒＝演台で話す担当。⚠️ #05 171〜216秒は**英字の図**
    #   （"KSC 1st Opportunity… Deorbit Burn - 7:15 am CST"）なので使わない。
    "c418": dict(clip="fd16", start=40.0, until=170.0),
    #
    # ── `mc0201`（2月1日の管制室。646×472・黒帯なし）────────────────
    #   #73 574〜588秒＝管制室の正面（「Mission Control Center」の札と航跡の画面）
    #   #84 648〜667秒＝正面の画面に引いた航跡（19秒。いちばん長い）
    #   #88 675〜686秒＝飛行主任の席（"FLIGHT DIRECTOR" の札と3人）
    "c622": dict(clip="mc0201", start=575.0, until=587.0),
    "c713": dict(clip="mc0201", start=676.0, until=685.0),
    "c727": dict(clip="mc0201", start=649.0, until=666.0),
    #
    # ── `fdcomm`（飛行主任の通話記録。器1280 の中の絵は984）──────────
    #   #39 288〜306秒＝飛行主任の列（"FLIGHT DIRECTOR" の札と4人）
    #   #111 875〜891秒＝受話器を取る管制官
    #   #43 328〜340秒＝二人の管制官（顔の寄りではない）
    "c701": dict(clip="fdcomm", start=289.0, until=305.0),
    "c720": dict(clip="fdcomm", start=876.0, until=890.0),
    "c724": dict(clip="fdcomm", start=329.0, until=339.0),
    #
    # ── `mct`（再突入の再現。1920×1080）──────────────────────────
    #   ⚠️ 4分割のうち**左2/3は英字だらけの技術図**。右上の3D再現だけを切り出す。
    #      切り出しの窓＝`cuts/c7.py` の `trim`（0.706, 0.005, 0.998, 0.556）。
    #      その窓を額の箱にぴったり合わせる寄り＝下の zoom/xbias/bias（手で置かず、
    #      `fit()` の式から逆算した。cw=560.6 ch=595.1 ＝窓の画素数と一致）。
    #   🔴🔴 **3D再現の窓にも黄色い英字が出る時間帯がある**（実測）。
    #      129秒＝州名「UTAH」「CALIFORNIA」／190秒＝説明の字幕
    #      "only brightened for 0.3 seconds."／300秒＝州名「TEXAS」。
    #      **340〜470秒は1字も出ない**（340・350・362・372・390・420・440・470 で確認）。
    #      ＝ mct を当てる4欄は**すべてこの帯の中**に置く。ショット#2（188〜491秒）の内側。
    #      ⚠️ 時刻は名乗らない（この帯は GMT 13:57〜14:00＝EST 8:57〜9:00 にあたるので、
    #         8時53分のカットに当てる絵に時刻を書くと嘘になる。副題は「再現」とだけ言う）。
    "c711": dict(clip="mct", start=341.0, until=352.0,
                 zoom=1.8149, xbias=0.997, bias=0.011),
    "c715": dict(clip="mct", start=440.0, until=452.0,
                 zoom=1.8149, xbias=0.997, bias=0.011),
    "c717": dict(clip="mct", start=360.0, until=374.0,
                 zoom=1.8149, xbias=0.997, bias=0.011),
    "c705": dict(clip="mct", start=420.0, until=430.0,
                 zoom=1.8149, xbias=0.997, bias=0.011),
    #
    # ── `guncam`（陸軍ヘリの照準カメラ。640×480）─────────────────
    #   #05 30〜44秒＝いくつにも分かれた光の筋。**9秒以降**（0〜9秒は英語の表紙＝NOGO）
    #   ⚠️ HUD の英字は `cuts/c7.py` の `trim` で上下を落とす。ここでは `bias` で
    #      同じ窓（0.27〜0.855）を動画のコマにも当てる。
    "c725": dict(clip="guncam", start=30.4, until=43.0, bias=0.651),
    #
    # ── `tank`（2011年の記録映像。1280×720・黒帯なし）────────────────
    #   ⚠️ タンクが写るのは **#2 12〜22秒** と **#4 34〜44秒** だけ（`NOGO` 参照）。
    "ep01": dict(clip="tank", start=12.2, until=21.0),
    # ⚠️ ep02 は尺 9.41秒に対して窓が10秒しかない。`rate` で 0.93倍速にして
    #    尻の1秒（ショットの切れ目）を踏まないようにした（タンクの絵なので顔は無い）。
    "ep02": dict(clip="tank", start=34.1, until=43.0, rate=0.93),
    # KB_USE>>> ここまで
}
