...
def report(d):
    ...
    for p in files:
        ...
        holes.append(hole)
        flag = ""
        # 🔴🔴 2026-09-07（5本目 SL-1 ⑤c'）：**占有率 100%／空き 0% は「満点」ではなく
        #    「レイヤーが壊れている」の合図。** c724 の `_lab` を焼いている途中で
        #    Chrome が落ち、`c724_base.png` が**真っ白の不透明1枚**になった。
        #    合成すると画面が真っ白になり、地との差が全画素で出るので
        #    **この検査だけが 100.0% / 0.0% と最高点を出す**（机上検査は絵を見ないので黙る）。
        #    直したあと同じカットは 47.2% / 12.3%。→ 疑わしいので必ず名指しする。
        if fill > 0.995 or (fill > 0.98 and hole < 0.005):
            flag += " 🔴レイヤーが壊れている疑い（占有率が高すぎる）"
        if fill < FILL_MIN:
            flag += " 🔴占有率"
        if hole > HOLE_MAX:
            flag += " 🔴空き矩形"
