...
# 🔴🔴 2026-09-13（7本目⑤b）：**7本目「2001年9月11日 米国同時多発テロ」へ差し替え**
#    （台本第2版 §4 の章見出し。**9章**＋冒頭の引き＋締め）。
#    6本目（キー橋）の章名は git の `121d3be` にある。
#    ⚠️ 7本目は `ca`（第10章）が**無い**。NCH も 10→9 に戻す。
...
#    正本＝Vault `事故検証-テネリフェ-台本第2版-20260916.md` §4 の章見出し
#    （＝`narration.py` の `# ── 第N章　…`。`（` の手前まで）。
...
# 🔴 2026-09-24（13本目 ⑤b-1）：**空にした**（§0b の12か所目）。12本目の9件は git の `3832147`。
#    ⚠️ 12本目の「その後」は13本目の第9章と同じ名前＝残すと12本目の色が黙って当たる。
#    13本目の色はカズヤくんとの相談（⑤b-1）で決めてからここに書く。空のあいだは全章 navy。
CHAPTER_PALETTE = {}
...
EP7_PHOTO = {
    ...
    "ep7/artcc_screen_pre.jpg": "出典：アメリカ連邦航空局（FAA）／パブリックドメイン",
    ...
    "ep7/f15_alert2.jpg": "出典：U.S. Air Force photo by Tech. Sgt. Michael Charles／パブリックドメイン",
