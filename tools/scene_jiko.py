...
def c2_lab():
    ...
    # ⚠️ 18巡目：5.5m の寸法線を y=330 に置いていたので、測っている剥離部（y=596）から
    #    267px も離れて浮いていた（垂直尾翼の高さに合わせてしまっていた）。
    #    寸法線は**測る物のすぐ上**に置く。剥離部の真上 y=552 に降ろす。
    g = [J.dim(ax(120), ax(249.5), 552, "5.5 m"),
...
def c2_a1():
    ...
    """
    return J.label((ax(120) + ax(249.5)) / 2, 484, "剥離した範囲", J.ALERT, 52, "middle")
