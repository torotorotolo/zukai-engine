...
GATES = [
    # 🔴 いちばん先。**章ファイルが読めていないと、後ろの門番は空の SPEC を調べて全部 ✓ を出す**
    #    （2026-09-08 に実際に起きた。循環参照で2章が落ちたまま check_layout が「✓」）
    ("cuts", ["tools/check_cuts.py"], "章ファイルが読めていて台本と1対1か・写真20%以上（警告だけ）"),
    # 🔴🔴 2026-09-09（6本目 ⑤c 追加）**画面の隅に出る章名が前作のままでないか。**
    #    判定そのものは `check_final.py` に前から在った。ただしあれは**本編 mp4 を要る作り**で
    #    ⑥ でしか回らず、ここに載っていなかったので **⑤b の門番11本は1本も鳴らなかった**。
    #    6本目は 216カット全部の隅が「1/9 北極でも動く炉」＝**5本目 SL-1 の章名のまま**で、
    #    ⑤c で1枚焼いて目で見るまで誰も気づいていない（10章のうち第10章は欄ごと無かった）。
    #    → mp4 の要らない `--chapters` を新設してここへ移した。
    #    ⚠️ 単発で回す門番は忘れられる（[[feedback-gates-blind-spot-is-the-scan-direction]]）。
    ...
    # 🔴 13本目から（⑤b-2・09-24）：仕組みの動く模式図（錠・断面）の状態の筋と部品の画素と札の数
    ("mech", ["tools/check_mech.py"], "仕組みの模式図：状態の筋（報告書）・描いた部品の位置・札の数の宣言"),
    ("slide", ["tools/check_slide.py"], "焼き込みの文字×切り方（G-09/10/13/14/15）"),
    # 🔴 12本目 ⑤c' から：画面に出る窓の上下の辺が字の行を横に切っていないか（slide の apply_trim は詰めるので映らない型）
    ("edges", ["tools/check_page_edges.py"], "画面に出る辺が字の行を横に切っていないか（頁の切り方・寄りの窓）"),
    ("blank", ["tools/check_blank.py"], "切り出し窓が空っぽでないか"),
    ("wrap", ["tools/check_wrap.py"], "行が語の途中で割れていないか（L-15）"),
    # 🔴🔴 2026-09-22（11本目 ⑥ の試写）**40件の指摘のうち13件が数字の誤読だった。**
    #    数字は Scribe が「1984年」をどう読んでも「1984年」と書き戻すので、
    #    **聞取でも一致率でも原理的に捕まえられない**（10本目「502人＝ごひゃくふたり」と同じ形）。
    #    ⑤a の機械の網（el_check_yomi・el_retake・el_extra_sound…）は**全部通っていた**。
    #    → 「送信文字列に生の数が残っていないか」だけは機械で見られるので、ここで見る。
    #    ⚠️ 回の名前は渡さない（`el_script.SLUG` から取る＝前作の名前が残らない）。
    ("yomi_num", ["tools/check_yomi_numbers.py"],
     "読みが固定されていない数（ElevenLabs）／数の台帳×音声記号列（ゆっくり＝check_aq_yomi へ振り分け）"),
    # 🔴 2026-09-25（14本目⑤a 新設）**最小限の聞き役**（ルール §4-15）。数の決まりを ④' は数えるだけだった
    #    ＝⑤以降に行を直すと黙って崩れる。聞き役の無い回（13本目まで）は「対象外」で exit 0。
    ("listener", ["tools/check_listener.py"], "聞き役：数字0・空き90秒まで・roles.tsv と1行ずつ（割合と回数は W）"),
