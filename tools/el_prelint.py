...
PATTERNS = [
    ...
    # ── 7本目 9.11（2026-09-13）。台本第2版 §6 の候補と概要欄 §11-5 の用語のうち、**実際に本文に出る語だけ**
    #    （件数は el_script.lines() に当てて数えた。出ない語＝アンドルーズ・スクランブル・飛行記録装置・
    #     シークレットサービス・EDT は外した。⚠️ アンドルーズは「画」の欄にしか無い＝音声には出ない）。
    #    §6 に無いが同じ型で危ない語（当直・手引き・迎撃・撃墜・無線・交信・機体・乗員）も足した。
    ("§6の語（9.11）", re.compile(r"委員会報告|委員会|航空路|管制|防空司令部|応答符号|跳ね返り|北棟|南棟|駐機場"
                                 r"|ニューアーク|ダレス|オーティス|ラングレー|シャンクスビル|スライニー"
                                 r"|共通戦略|西南西|原則|トランスポンダ|一次レーダー|二次レーダー|コマンドセンター"
                                 r"|北東航空防衛セクター|連邦航空局|連邦航空保安官|緊急発進|音声記録装置|記録装置"
                                 r"|大統領警護隊|操縦室|客室乗務員|乗員|当直|手引き|迎撃|撃墜|無線|交信|機体"
                                 r"|世界貿易センター|ペンタゴン|国防総省|自動評価|自動操縦|保安検査|独立調査委員会")),
    # ── 8本目 コロンビア号（2026-09-14）。台本第2版 §6 の候補と概要欄 §11 の用語のうち、
    #    **実際に本文（el_script.lines() の489行）に出る語だけ**を書いた（件数を数えてから足した）。
    #    ⚠️ §6 に載っているが本文に出ない語は入れない＝**マッハ（④'で全廃・本文0件）**・FREESTAR（落とした）・
    #       前縁（「前のふち」に統一）。入れると「当たらないキー」がノイズになる。
    #    §6 に無いが同じ型で危ない語（打ち上げ・軌道・断熱材・翼・貨物室・耐熱・回収）も足した。
    ("§6の語（コロンビア号）", re.compile(
        r"RCC|射点|前のふち|桁|オービタ|バイポッド|スペースハブ|クレーター"
        r"|ハズバンド|マクール|アンダーソン|ブラウン|チャウラ|クラーク|ラモン"
        r"|ロッチャ|マデラ|ハム|マイア|クレネク|ヤング|クリッペン"
        r"|ナコドーチェス|ヘンフィル|リトルフィールド|フォートポーク|フォートワース"
        r"|アルバカーキ|サクラメント|エドワーズ|ケネディ宇宙センター|ヒューストン"
        r"|強化炭素|炭素|断熱材|外部タンク|固体ロケット|貨物室|耐熱|翼|軌道|打ち上げ"
        r"|再突入|回収|飛行管理|画像取得|国防総省|委員会|報告書|温度|圧力|センサ")),
    # ── 9本目 テネリフェ（2026-09-16）。台本第2版 §6 の候補のうち、**実際に本文（495行）に出る語だけ**
    #    （件数は el_script.lines() に当てて数えた）。⚠️ §6 の「方位040」は本文に0件（c711-2 は「決められた方位」）。
    #    §6 に無いが同じ型で危ない語（管制官・管制塔＝7本目16/35行・8本目4/5行が崩れた「管制」／離陸＝8本目で
    #    かな化が『リリック』に悪化／副操縦士・航空機関士・操縦室・無線・復唱）も足した。
    ("§6の語（テネリフェ）", re.compile(
        r"一機|二機|管制官|管制塔|滑走路|誘導路|離陸|着陸|露点|中心線灯|復唱|バックタクシー|迂回"
        r"|ラスパルマス|グランカナリア|テネリフェ|カナリア諸島|スキポール|パンナム|クリッパー"
        r"|フェルトハイゼン|ファン・ザンテン|ミュールス|スフルーダー|グラブス|ブラッグ|ワーンズ"
        r"|ブラーテンス|スターリング|行った|行って|フライトレベル|方位|操縦室|航空機関士|副操縦士"
        r"|無線|甲高い|三番目|書き起こし")),
    # ── 10本目 三豊百貨店（2026-09-20）。台本第2版 §6-1 の候補のうち、**実際に本文（460行）に出る語だけ**
    #    （件数は el_script.lines() に当てて数えてから足した）。⚠️ §6-1 に載っていても本文に0件の語は入れない＝
    #       内力壁・引張鉄筋・無梁板・高法・定着（④'が全部言い換えた）。入れると「当たらないキー」がノイズになる。
    #    §6-1 に無いが同じ型で危ない語も足した:
    #      ・**柱／床／壁／板／棟／階** … この回の主役の名詞。4本目で「床」が **とこ** と読まれた実績がある
    #      ・**監理／管理** … 同音異義（④'が並ぶ所だけ和語に逃がした。残った2行は音で確かめる）
    #      ・**梁** … 「りょう」と読まれうる。8行に出る
    ("§6の語（三豊百貨店）", re.compile(
        r"三豊|瑞草区|冷却塔|押えコンクリート|防水層|請負限度制|竣工|監理|管理|梁|スラブ"
        r"|せん断|大法院|業務上過失致死傷|共同正犯|禁錮|執行猶予|控訴|上告|災難管理法|国政監査|白書"
        r"|焼香所|戸籍|貴金属|ウォン|区民会館|百貨店|建設産業|設計事務所|下請け"
        r"|柱|床|壁|板|棟|階|厨房|屋上|盛り上が|ひび|亀裂|補強|鉄筋|コンクリート")),
    # 🔴 2026-09-16（9本目）: 前の回で**実際に崩れた数の型**を2つ、題材によらない型として足した。
    #    ・桁区切りの数 … 7本目「26,000→二十六千」「10,700→十千七百」（**万の位が落ちる**）／
    #      8本目「38,500キログラム」は ⑤a の網を抜けて**試写でカズヤくんの耳**が拾った
    #    ・分の（分数） … 8本目 c410-2「50分の1」が『ごじゅっ**ぷん**のいち』（**時間の分**に読んだ）。これも試写で発見
    ("桁区切りの数",   re.compile(r"[0-9０-９]{1,3}(?:[,，][0-9０-９]{3})+")),
    ("分数（分の）",   re.compile(r"[0-9０-９]+分の[0-9０-９一二三四五六七八九十]")),
    ...
    # 🔴 2026-09-13（7本目 9.11）: **全角の丸括弧 （ ） を白名簿に足した。**
    #    2026-09-12 に「本文はメートル法（メートルを先・原単位は括弧）」が恒久ルールになったので、
    #    本文に括弧が必ず出る（この回は4行＝c113-1・c417-1・c623-3・c904-1 の単位換算だけ）。
    #    ⚠️ これは**門番をかわす綴り**ではない。この門番の狙いは「台本の注記（⚠️🔴✅🔧★）が字幕に紛れる」ことで、
    #       括弧は日本語の普通の約物。狙いの型が今も鳴ることは selftest の陽性対照で押さえてある。
    #    ⚠️ ただし**括弧を声がどう読むかは別の話**＝⑤a の A/B で確かめる（間が空く／読み飛ばす）。
    # 🔴 2026-09-14（8本目 コロンビア号）: **乗算記号 × （U+00D7）を白名簿に足した。**
    #    本文に1行だけ出る＝`c220-2`「最初は1983年。約48×30センチが欠けている。」（寸法の × ）。
    #    ⚠️ これも**門番をかわす綴りではない**。狙いは「台本の注記（⚠️🔴✅🔧★）が字幕に紛れる」ことで、
    #       寸法の × は日本語の普通の約物。狙いの型が今も鳴ることは selftest の陽性対照5本で押さえてある。
    #    ⚠️⚠️ **音は別の話。**× を声がどう読むかは決まっていない（読み飛ばす／「かける」／「エックス」）。
    #       ⑤a の A/B で確かめて EL_YOMI で固定する。**字幕は「48×30センチ」のまま**。
    #    ⚠️ 台本側の所見として⑥へ送る: この回の本文の寸法は他はすべて言葉書き
    #       （「長さ約61センチ、幅約38センチ」「縦横40センチほど」＝④'が c907 で × から直した形）。
    #       **c220-2 だけが × で不揃い。**画としては読めるので⑤aでは直さない（台本は正本）。
    # 🔴 2026-09-23（12本目 キャッスル・ブラボー⑤a）: **全角コロン ：（U+FF1A）を白名簿に足した。**
    #    ★決め所の4行が「予報：…」「灰：…」「委員長：…」「日本側：…」＝**誰の言葉かの札**（④'の書き方・画面にもこの形で出る）。
    #    × と同じく**門番をかわす綴りではない**（狙いの注記記号は今も陽性対照5本で鳴る）。
    #    ⚠️ 音は別の話＝声が ： を間にするか読み飛ばすかは決まっていないので、送る文字列だけ EL_YOMI で「、」に固定した。
...
def selftest() -> int:
    cases = [
        ...
        # 9本目（2026-09-16）で足した2型の陽性対照＝前の回で**実際に崩れた実文**
        ("38,500キログラムと84,900ポンド", {"桁区切りの数"}),
        ("燃料の50分の1が、そこで減った。", {"分数（分の）"}),
        ("実際に入れた量は、55,500リットル。", {"桁区切りの数"}),
        ("地面についた雲は、8分の1から8分の7へ増えた。", {"分数（分の）"}),
    ]
    # 陰性対照（9本目）＝**時間の「分の」**と、桁区切りでない数で鳴ってはいけない（9本目の実文）
    for text, notwant in [("26分のあいだに、3キロが300メートルになった。", "分数（分の）"),
                          ("25分の飛行に、多すぎる。", "分数（分の）"),
                          ("1977年3月27日の夕方5時すぎ。", "桁区切りの数"),
                          ("17時06分50秒、二機はぶつかった。", "桁区切りの数")]:
        if notwant in {n for n, _ in scan_line(text)}:
            fails.append((text, f"陰性対照＝{notwant} は鳴らないはず", notwant))
    fails = []
    ...
    for text in ["かぶりは、¾インチ。「SL-1」という名前である…",
                 ...
                 # ↓ 7本目 9.11 の実文（**全角の丸括弧**が出る4行のうち3行。単位のメートル換算＝2026-09-12 の恒久ルール）
                 "当時は、刃渡り10センチ（4インチ）未満の刃物なら、",
                 "点は、ペンタゴンの西南西8キロ（5マイル）まで来ていた。",
                 "首都まで、およそ201キロ（125マイル）",
                 # ↓ 8本目 コロンビア号の実文（**乗算記号 ×** が出る唯一の行＝c220-2）
